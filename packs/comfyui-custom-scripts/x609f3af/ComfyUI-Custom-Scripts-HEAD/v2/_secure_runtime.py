"""Runtime helpers for the pinned Custom Scripts secure conversion."""
...
def schema_for(node_id: str) -> io.Schema:
    ...
    # No converted node needs the legacy prompt/workflow snapshots. Keeping
    # them out is both the secure boundary and avoids injecting arguments the
    # converted handlers do not accept.
    attrs["hidden"] = []
    if node_id == "CheckpointLoader|pysssss":
        attrs["inputs"][0].remote = io.RemoteOptions(
            route="/models/checkpoints", refresh_button=True
        )
    elif node_id == "LoraLoader|pysssss":
        attrs["inputs"][2].remote = io.RemoteOptions(
            route="/models/loras", refresh_button=True
        )
    elif node_id == "LoadText|pysssss":
        binding = attrs["inputs"][1].extra_dict["pysssss.binding"][0]
        for callback in binding["callback"]:
            if callback.get("type") == "fetch":
                callback["url"] = (
                    "/secure-nodes/text-files/{$source.value}"
                )
    elif node_id == "SaveText|pysssss":
        root = attrs["inputs"][0]
        root.options = ["output", "temp"]
        root.default = "output"
    elif node_id == "Repeater|pysssss":
        attrs["enable_expand"] = True
        source, repeats, output, _node_mode = attrs["inputs"]
        source.rawLink = True
        repeats.max = 100
