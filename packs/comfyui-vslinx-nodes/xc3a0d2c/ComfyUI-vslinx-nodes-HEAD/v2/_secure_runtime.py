"""Frozen schemas and wire helpers for the pinned vsLinx conversion."""
...
def schema_for(node_id: str) -> io.Schema:
    ...
    if node_id in {
        ...
        index = next(
            index for index, item in enumerate(attrs["inputs"])
            if item.id == "lllite_name"
        )
        original = attrs["inputs"][index]
        attrs["inputs"][index] = io.Combo.Input(
            "lllite_name",
            options=[],
            default="",
            optional=False,
            ...
            remote=io.RemoteOptions(
                route="/models/controlnet",
                refresh_button=True,
            ),
        )
        if node_id == "vsLinx_AnimaLLLiteLoader":
            attrs["outputs"] = [io.Combo.Output(
                "output_0", display_name="lllite_name")]
