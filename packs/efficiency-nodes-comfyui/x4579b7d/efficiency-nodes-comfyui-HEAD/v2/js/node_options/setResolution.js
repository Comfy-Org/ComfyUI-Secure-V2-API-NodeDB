...
// One registration per loader: the preset list is keyed by node type, which the
// old callback looked up from the node the menu was opened on and which a
// per-type registration knows outright.
for (const [nodeType, resolutions] of Object.entries(RESOLUTIONS)) {
    comfy.defs.extend(nodeType, (b) => {
        b.addMenuItem({
            ...
            items: resolutions.map(res => ({
                ...
                run: (node) => setNodeResolution(node, res.width, res.height)
            }))
        });
    });
}
