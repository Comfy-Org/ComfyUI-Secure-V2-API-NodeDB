...
def create_core_rife(state_dict: dict[str, torch.Tensor]) -> EnhancedCoreRife:
    """Load current RIFE weights with the reviewed vendored IFNet class."""

    from . import ifnet

    detector: Any = ifnet.detect_rife_config
    head_channels, channels = detector(state_dict)
    network = cast(nn.Module, ifnet.IFNet(head_ch=head_channels, channels=channels))
