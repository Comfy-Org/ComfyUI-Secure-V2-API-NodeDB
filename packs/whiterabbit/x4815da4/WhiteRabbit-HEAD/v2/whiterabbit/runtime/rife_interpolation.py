...
class RifeInterpolationEngine:
    ...
    def synthesize_pair(
        ...
        if loaded.spec.architecture == "core":
            ...
            height = ((image_0.shape[-2] - 1) // alignment + 1) * alignment
            width = ((image_0.shape[-1] - 1) // alignment + 1) * alignment
            padding = (0, width - image_0.shape[-1], 0, height - image_0.shape[-2])
            image_0 = torch.nn.functional.pad(image_0, padding, mode="replicate")
            image_1 = torch.nn.functional.pad(image_1, padding, mode="replicate")
    ...
    def _progress_bar(total: int) -> ProgressBar:
        """Use an inert local counter; the async wrapper reports progress."""
    ...
    def _clear_cache() -> None:
        """Release only memory owned by this pack process."""
