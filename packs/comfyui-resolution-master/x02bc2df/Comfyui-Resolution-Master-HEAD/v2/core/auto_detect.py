...
class _PackLogger:
    """Keep upstream's multi-part log calls on the standard logger."""
    ...
    def debug(self, *parts):
        self._logger.debug(self._message(parts))

    def info(self, *parts):
        self._logger.info(self._message(parts))

    def warning(self, *parts):
        self._logger.warning(self._message(parts))


log = _PackLogger(__name__)
