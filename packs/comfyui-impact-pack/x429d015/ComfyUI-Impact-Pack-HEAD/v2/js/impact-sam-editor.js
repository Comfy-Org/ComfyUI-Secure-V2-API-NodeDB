...
// Handles hold no arbitrary properties, so the node the editor was opened from
// — the old `ComfyApp.clipspace_return_node` — lives here.
...
async function uploadMask(filepath, formData) {
	await comfy.backend.fetch('/upload/mask', {
		method: 'POST',
		body: formData
	...
	// REFUSED, not a pending gap: the uploaded mask was also pushed back into
	// `ComfyApp.clipspace.imgs` / `.images` and core's preview invalidated with
	// `ClipspaceDialog.invalidatePreview()`. Clipspace data stays unpublished;
	// the mask is written onto the return node's `image` widget in save().
}

class ImpactSamEditorDialog {
	...
	constructor() {
		...
		for(const child of this.createButtons()) {
	...
	show(target_image_path) {
		this.mask_image = null;
		self.prompt_points = [];

		this.message_box = document.createElement("p");
		this.message_box.textContent = "Please wait a moment while the SAM model and the image are being loaded.";
		...
		if(!this.is_layout_created) {
			...
			// REFUSED, not a pending gap: the MutationObserver here existed only
			// to call `ComfyApp.onClipspaceEditorClosed()` when the dialog was
			// hidden, and clipspace has no published lifecycle to report to.
		}
	...
	async load_sam(url) {
		throw new Error('SAM editor needs a closed host detector primitive');
	}
	...
	async save(self) {
		...
		// The published replacement for `ComfyApp.clipspace_return_node` plus
		// `ComfyApp.clipspace.widgets['image'].value`: write the mask straight
		// onto the node the menu was opened from.
		const image_widget = return_node?.widgets.get('image');
		if(image_widget)
			image_widget.setValue(`${filename} [temp]`);

		this.close();
	}
}

// REFUSED, not a pending gap: `ClipspaceDialog.registerButton("Impact SAM
// Detector", ...)` added a second entry point from inside core's clipspace
// editor. Clipspace stays unpublished, so only the node menu entry below
// remains.

// No SAM menu item is registered in the secure build: its old endpoints
// loaded live detector objects and held cross-request process-global state.
