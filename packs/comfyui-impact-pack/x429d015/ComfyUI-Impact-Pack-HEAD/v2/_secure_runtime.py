...
def schema_for(node_id: str) -> io.Schema:
    ...
    if node_id in {"ImpactSwitch", "LatentSwitch", "SEGSSwitch"}:
        first = next(item for item in attrs["inputs"] if item.id == "input1")
        attrs["inputs"] = [
            item for item in attrs["inputs"] if item.id != "input1"
        ]
        names = [f"input{index}" for index in range(1, 101)]
        template = io.Autogrow.TemplateNames(first, names=names, min=1)
        attrs["inputs"].append(io.Autogrow.Input("inputs", template=template))
        next(item for item in attrs["inputs"] if item.id == "select").max = 100

    if node_id == "ImpactInversedSwitch":
        original = attrs["outputs"][0]
        attrs["outputs"] = []
        for index in range(1, 101):
            output = copy.copy(original)
            output.id = f"output{index}"
            attrs["outputs"].append(output)
        next(item for item in attrs["inputs"] if item.id == "select").max = 100
...
async def _wrap_seg_elt(value: Any) -> Any:
    """Keep SEGS wire-safe by replacing nested tensors with opaque refs."""
    if value is None:
        return None
    if isinstance(value, dict):
        fields = (
            "cropped_image", "cropped_mask", "confidence", "crop_region",
            "bbox", "label", "control_net_wrapper",
        )
        items = [value.get(field) for field in fields]
    elif isinstance(value, (tuple, list)) and len(value) == 7:
        items = list(value)
    else:
        raise TypeError("SEG_ELT must use Impact Pack's seven-field layout")
    items[0] = await wrap_output(items[0], "IMAGE")
    items[1] = await wrap_output(items[1], "MASK")
    return tuple(items)


async def _wrap_segs(value: Any) -> Any:
    if not isinstance(value, (tuple, list)) or len(value) != 2:
        raise TypeError("SEGS must be a (shape, segments) pair")
    return tuple(value[0]), [await _wrap_seg_elt(item) for item in value[1]]
...
async def wrap_output(value: Any, io_type: str, is_list: bool = False) -> Any:
    ...
    if io_type == "SEGS":
        return await _wrap_segs(value)
    if io_type == "SEG_ELT":
        return await _wrap_seg_elt(value)
    return value
