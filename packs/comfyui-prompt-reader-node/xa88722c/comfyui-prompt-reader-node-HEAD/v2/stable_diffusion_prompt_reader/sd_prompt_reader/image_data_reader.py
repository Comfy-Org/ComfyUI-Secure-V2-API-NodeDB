...
class ImageDataReader:
    ...
    @staticmethod
    def _user_comment(image):
        """Decode the bounded EXIF UserComment formats accepted upstream."""
        try:
            payload = image.getexif().get_ifd(0x8769).get(0x9286)
            ...
            try:
                ...
                if f.format == "PNG":
                    ...
                    if "comment" in self._info:
                        ...
                        try:
                            user_comment = self._user_comment(f)
                            ...
                            try:
                                # swarm format
                                if "sui_image_params" in user_comment:
                                    self._tool = "StableSwarmUI"
                                    self._parser = SwarmUI(raw=user_comment)
                                else:
                                    self._raw = user_comment
    ...
    def save_image(image_path, new_path, image_format, data=None):
        ...
        if data:
            match image_format.upper():
                ...
                case "JPEG" | "JPG" | "WEBP":
                    metadata = b"UNICODE\x00" + data.encode("utf-16-be")
                ...
                match image_format.upper():
                    ...
                    case "JPEG" | "JPG":
                        ...
                        if data:
                            ...
                            exif_ifd[0x9286] = metadata
                            exif[0x8769] = exif_ifd
                        f.save(new_path, quality="keep", exif=exif)
                    case "WEBP":
                        exif = f.getexif()
                        if data:
                            exif_ifd = exif.get_ifd(0x8769)
                            exif_ifd[0x9286] = metadata
                            exif[0x8769] = exif_ifd
                        f.save(new_path, quality=100, lossless=True, exif=exif)
            except Exception:
                print("Save error")
