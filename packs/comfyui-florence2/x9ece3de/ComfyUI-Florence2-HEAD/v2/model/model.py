...
from . import _ops
from ._ops import ProgressBar, attention_small_input
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, key_value_states=None, past_key_value=None, attention_mask=None):
        ...
        attn_output = attention_small_input(query_states, key_states, value_states, self.num_heads, mask=attention_mask, skip_reshape=True, skip_output_reshape=True)
...
class CosinePositionEmbedding1D(nn.Module):
    ...
    def forward(self, seq_embeds):
        ...
        pos_embeds = _ops.cast_to_input(self.pos_idx_to_embed[:seq_embeds.size(-2), :], seq_embeds)
        if shape_len == 3:
