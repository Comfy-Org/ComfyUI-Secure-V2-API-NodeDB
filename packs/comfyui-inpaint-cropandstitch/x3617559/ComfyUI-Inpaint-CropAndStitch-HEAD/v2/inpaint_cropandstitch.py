...
OUTPUT_PADDING = ["0", "8", "16", "32", "64", "128", "256", "512"]
STITCHER = io.Custom("STITCHER")

_STITCHER_SCALAR_KEYS = {
    "downscale_algorithm",
    "upscale_algorithm",
    "blend_pixels",
    "device_mode",
}
_STITCHER_NUMBER_LIST_KEYS = {
    "canvas_to_orig_x",
    "canvas_to_orig_y",
    "canvas_to_orig_w",
    "canvas_to_orig_h",
...
_STITCHER_IMAGE_LIST_KEY = "canvas_image"
_STITCHER_MASK_LIST_KEY = "cropped_mask_for_blend"
_STITCHER_KEYS = (
    _STITCHER_SCALAR_KEYS
    | _STITCHER_NUMBER_LIST_KEYS
    | {_STITCHER_IMAGE_LIST_KEY, _STITCHER_MASK_LIST_KEY}
)


def _to_pil_image(sample: torch.Tensor) -> Image.Image:
    """Convert a CHW float tensor to PIL without importing torchvision."""
    value = sample.detach().cpu().movedim(0, -1).numpy()
    if value.shape[-1] == 1:
        value = value[..., 0]
    return Image.fromarray((value * 255.0).astype(np.uint8))


def _from_pil_image(image: Image.Image) -> torch.Tensor:
    value = np.array(image, copy=True)
    if value.ndim == 2:
        value = value[..., None]
    return torch.from_numpy(value).movedim(-1, 0).float().div(255.0)


async def _placed_image_value(
    image: sdk.ImageRef,
    device_mode: str,
) -> torch.Tensor:
    if not isinstance(image, sdk.ImageRef) or image.kind != "IMAGE":
        raise TypeError("image must be a typed IMAGE ref")
    if device_mode not in DEVICE_MODES:
        raise ValueError(f"unsupported device_mode {device_mode!r}")
    placement = "gpu" if device_mode == "gpu (much faster)" else "cpu"
    return await (await image.to_device(placement)).raw()


async def _optional_mask_value(
    mask: sdk.TensorRef | None,
    device: torch.device,
    *,
    name: str,
) -> torch.Tensor | None:
    if mask is None:
        return None
    if not isinstance(mask, sdk.TensorRef) or mask.kind not in {"MASK", "IMAGE"}:
        raise TypeError(f"{name} must be a typed MASK ref")
    return (await mask.raw()).to(device)


async def _stitcher_with_refs(stitcher: dict) -> dict:
    converted = dict(stitcher)
    converted[_STITCHER_IMAGE_LIST_KEY] = [
        await sdk.ImageRef._from_raw(value)
        for value in stitcher[_STITCHER_IMAGE_LIST_KEY]
    ]
    converted[_STITCHER_MASK_LIST_KEY] = [
        await sdk.MaskRef._from_raw(value)
        for value in stitcher[_STITCHER_MASK_LIST_KEY]
    ]
    return converted


def _validate_stitcher(stitcher: object) -> dict:
    if not isinstance(stitcher, dict) or set(stitcher) != _STITCHER_KEYS:
        raise TypeError("stitcher must have the exact bounded STITCHER shape")
    if stitcher["downscale_algorithm"] not in RESIZE_ALGORITHMS:
        raise ValueError("stitcher has an unsupported downscale algorithm")
    if stitcher["upscale_algorithm"] not in RESIZE_ALGORITHMS:
        raise ValueError("stitcher has an unsupported upscale algorithm")
    if stitcher["device_mode"] not in DEVICE_MODES:
        raise ValueError("stitcher has an unsupported device mode")
    blend_pixels = stitcher["blend_pixels"]
    if isinstance(blend_pixels, bool) or not isinstance(blend_pixels, int):
        raise TypeError("stitcher blend_pixels must be an integer")
    if not 0 <= blend_pixels <= 64:
        raise ValueError("stitcher blend_pixels is out of range")

    sequence_keys = (
        _STITCHER_NUMBER_LIST_KEYS
        | {_STITCHER_IMAGE_LIST_KEY, _STITCHER_MASK_LIST_KEY}
    )
    if any(not isinstance(stitcher[key], list) for key in sequence_keys):
        raise TypeError("stitcher batch members must be lists")
    sizes = {len(stitcher[key]) for key in sequence_keys}
    if len(sizes) != 1 or sizes == {0}:
        raise ValueError("stitcher batch members must have one equal nonzero size")
    for key in _STITCHER_NUMBER_LIST_KEYS:
        if any(isinstance(value, bool) or not isinstance(value, int)
               for value in stitcher[key]):
            raise TypeError(f"stitcher {key} members must be integers")
    if any(
        not isinstance(value, sdk.ImageRef) or value.kind != "IMAGE"
        for value in stitcher[_STITCHER_IMAGE_LIST_KEY]
    ):
        raise TypeError("stitcher canvas_image members must be IMAGE refs")
    if any(
        not isinstance(value, sdk.TensorRef)
        or value.kind not in {"MASK", "IMAGE"}
        for value in stitcher[_STITCHER_MASK_LIST_KEY]
    ):
        raise TypeError("stitcher blend-mask members must be MASK refs")
    return dict(stitcher)


async def _materialize_stitcher(stitcher: object, device: torch.device) -> dict:
    value = _validate_stitcher(stitcher)
    value[_STITCHER_IMAGE_LIST_KEY] = [
        (await ref.raw()).to(device)
        for ref in value[_STITCHER_IMAGE_LIST_KEY]
    ]
    value[_STITCHER_MASK_LIST_KEY] = [
        (await ref.raw()).to(device)
        for ref in value[_STITCHER_MASK_LIST_KEY]
    ]
    return value
...
class CPUProcessorLogic(ProcessorLogic):
    def rescale_i(self, samples, width, height, algorithm: str):
        ...
        for i in range(samples.shape[0]):
            samples_pil = _to_pil_image(samples[i]).resize(
                (width, height), algorithm_enum
            )
            results.append(_from_pil_image(samples_pil))
    ...
    def rescale_m(self, samples, width, height, algorithm: str):
        ...
        results = []
        for i in range(samples.shape[0]):
            samples_pil = _to_pil_image(samples[i].unsqueeze(0)).resize(
                (width, height), algorithm_enum
            )
            results.append(_from_pil_image(samples_pil).squeeze(0))
        samples = torch.stack(results, dim=0)
        return samples
...
class GPUProcessorLogic(ProcessorLogic):
    def rescale_i(self, samples, width, height, algorithm: str):
        ...
        results = []
        for i in range(samples.shape[0]):
            samples_pil = _to_pil_image(samples[i].float()).resize(
                (width, height), algorithm_enum
            )
            results.append(_from_pil_image(samples_pil))
    ...
    def rescale_m(self, samples, width, height, algorithm: str):
        ...
        results = []
        for i in range(samples.shape[0]):
            samples_pil = _to_pil_image(samples[i].float().unsqueeze(0)).resize(
                (width, height), algorithm_enum
            )
            results.append(_from_pil_image(samples_pil).squeeze(0))
        samples = torch.stack(results, dim=0).to(original_device)
...
class InpaintCropImproved(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        ...
        return io.Schema(
            node_id="InpaintCropImproved",
            display_name="✂️ Inpaint Crop",
            category="inpaint",
            description=(
                "Crops an image around a mask for inpainting, the optional "
                "context mask defines an extra area to keep for the context."
            ),
            inputs=[
                io.Image.Input("image"),
                io.Combo.Input(
                    "downscale_algorithm",
                    options=RESIZE_ALGORITHMS,
                    default="bilinear",
                ),
                io.Combo.Input(
                    "upscale_algorithm",
                ...
                io.Int.Input(
                    ...
                    step=1,
                    tooltip=(
                        "Expand the mask by a certain amount of pixels before "
                        "processing."
                    ),
                ),
                io.Boolean.Input(
                ...
                io.Float.Input(
                    ...
                    step=0.01,
                    tooltip=(
                        "Grow the context area from the mask by a certain "
                        "factor in every direction. For example, 1.5 grabs "
                        "extra 50% up, down, left, and right as context."
                    ),
                ),
                io.Boolean.Input(
                ...
                io.Combo.Input(
                    ...
                    default="cpu (compatible)",
                ),
                io.Mask.Input("mask", optional=True),
                io.Mask.Input("optional_context_mask", optional=True),
            ],
            outputs=[
                STITCHER.Output("stitcher", display_name="stitcher"),
                io.Image.Output(
                    "cropped_image", display_name="cropped_image"
                ),
                io.Mask.Output("cropped_mask", display_name="cropped_mask"),
            ],
        )
    ...
    async def execute(
        ...
        image = (await _placed_image_value(image, device_mode)).clone()
        device = image.device
        mask = await _optional_mask_value(mask, device, name="mask")
        if mask is not None:
            mask = mask.clone()
            if mask.ndim == 2:
                mask = mask.unsqueeze(0)  # (H, W) -> (1, H, W)
        optional_context_mask = await _optional_mask_value(
        ...
        result_stitcher = await _stitcher_with_refs(result_stitcher)
        result_image_ref = await sdk.ImageRef._from_raw(result_image)
        result_mask_ref = await sdk.MaskRef._from_raw(result_mask)
        ...
        if self.DEBUG_MODE:
            ...
            final_debug_refs = []
            for output_type, value in zip(
                self.RETURN_TYPES[3:], final_debug_outputs, strict=True
            ):
                if output_type == "IMAGE":
                    final_debug_refs.append(await sdk.ImageRef._from_raw(value))
                else:
                    final_debug_refs.append(await sdk.MaskRef._from_raw(value))
        ...
        return io.NodeOutput(result_stitcher, result_image_ref, result_mask_ref)
...
class InpaintStitchImproved(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="InpaintStitchImproved",
            display_name="✂️ Inpaint Stitch",
            category="inpaint",
            description=(
                "Stitches an image cropped with Inpaint Crop back into the "
                "original image"
            ),
            inputs=[
                STITCHER.Input("stitcher"),
                io.Image.Input("inpainted_image"),
            ],
    ...
    @classmethod
    async def execute(cls, stitcher, inpainted_image) -> io.NodeOutput:
        stitcher = _validate_stitcher(stitcher)
        device_mode = stitcher["device_mode"]
        inpainted_image = (
            await _placed_image_value(inpainted_image, device_mode)
        ).clone()
        device = inpainted_image.device
        stitcher = await _materialize_stitcher(stitcher, device)
        results = []
        ...
        for i in range(batch_size):
            ...
            one_image, = cls.inpaint_stitch_single_image(
        ...
        return io.NodeOutput(await sdk.ImageRef._from_raw(result_batch))
