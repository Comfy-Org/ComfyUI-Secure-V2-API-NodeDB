"""Frozen schema and wire helpers for the pinned iTools V2 conversion."""
...
def schema_for(node_id: str) -> io.Schema:
    ...
    # The conversion does not need ambient prompt/workflow snapshots. Frontend
    # state travels only through the node's own bounded widget values.
    attrs["hidden"] = []
    if node_id in _DYNAMIC_INPUT_NODES:
        attrs["accept_all_inputs"] = True
    if node_id in {"iToolsLoadImagePlus", "iToolsCropImage"}:
        image = next(item for item in attrs["inputs"] if item.id == "image")
        image.options = []
        image.default = ""
    if node_id in {"iToolsLoadImages", "iToolsLoadRandomImage"}:
        directory = next(
            item for item in attrs["inputs"] if item.id == "images_directory"
        )
...
def bind_node(
    ...
    *,
    ...
    async def fingerprint(cls, **kwargs: Any):
        result = fingerprint_handler(**kwargs)
        return await result if inspect.isawaitable(result) else result

    async def validate(cls, **kwargs: Any):
        result = validate_handler(**kwargs)
        return await result if inspect.isawaitable(result) else result
    ...
    fingerprint_handler = getattr(handler, "fingerprint_inputs", None)
    if fingerprint_handler is not None:
        attrs["fingerprint_inputs"] = classmethod(fingerprint)
    validate_handler = getattr(handler, "validate_inputs", None)
