...
def schema_for(node_id: str) -> io.Schema:
    ...
    if node_id == "VHS_VideoCombine":
        original = attrs["inputs"][0]
        attrs["inputs"][0] = io.MultiType.Input(
            original.id, [io.Image, io.Latent],
            display_name=original.display_name,
            optional=original.optional,
            tooltip=original.tooltip,
        )
    if node_id in {
        "VHS_LoadVideo", "VHS_LoadVideoPath", "VHS_LoadVideoFFmpeg",
        "VHS_LoadVideoFFmpegPath", "VHS_LoadImagePath",
    }:
        original = attrs["outputs"][0]
        attrs["outputs"][0] = io.AnyType.Output(
            original.id, display_name=original.display_name,
            tooltip=original.tooltip,
            is_output_list=original.is_output_list,
        )
