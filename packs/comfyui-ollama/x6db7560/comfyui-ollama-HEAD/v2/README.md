...
> **Secure Nodes V2:** Ollama access is provided by the typed
> `integrations.ollama` host broker. Nodes may select only a loopback Ollama
> origin or an administrator-configured `ollama://` profile; they do not get
> ambient HTTP, filesystem, credentials, raw image tensors, or the Python
> `ollama` client. Model discovery uses the sandbox-safe frontend facade.
> Pack-local installation of `ollama` or `dotenv` is not required.
