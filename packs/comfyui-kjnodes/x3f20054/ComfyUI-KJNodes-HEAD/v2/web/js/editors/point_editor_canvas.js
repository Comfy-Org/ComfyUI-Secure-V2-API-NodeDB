...
import { rectHitTest, cursorForBboxMode } from '../utility.js';
...
comfy.defs.extend('PointsEditor', (b) => {
...
class PointsEditor extends BaseEditorCanvas {
  constructor(context, reset = false) {
    ...
    this._storeSubscriptions = [
      this.pointsStoreWidget.on('change', () => {
        try {
      ...
      this.bboxStoreWidget.on('change', () => {
        try { this.bbox = JSON.parse(this.bboxStoreWidget.getValue()); }
    ...
    if (!reset && this.pointsStoreWidget.getValue() !== "") {
      try {
        ...
        this.points = parsed.positive || [];
        this.neg_points = parsed.negative || [];
        this.bbox = JSON.parse(this.bboxStoreWidget.getValue());
      } catch (e) {
      ...
      this.points = [{ x: this.coordWidth / 2, y: this.coordHeight / 2 }];
      this.neg_points = [];
      this.pointsStoreWidget.setValue(JSON.stringify({ positive: this.points, negative: this.neg_points }));
      this.bboxStoreWidget.setValue(JSON.stringify(this.bbox));
    }
  ...
  destroy() {
    for (const unsubscribe of this._storeSubscriptions) unsubscribe();
    super.destroy();
  }
  ...
  updateData() {
    ...
    this.pointsStoreWidget.setValue(JSON.stringify(combinedPoints));
    this.pos_coordWidget.setValue(JSON.stringify(this.points || []));
    this.neg_coordWidget.setValue(JSON.stringify(this.neg_points || []));
    if (this.bbox.length !== 0) {
      let bboxString = JSON.stringify(this.bbox);
      this.bboxStoreWidget.setValue(bboxString);
      this.bboxWidget.setValue(bboxString);
    }
  }
