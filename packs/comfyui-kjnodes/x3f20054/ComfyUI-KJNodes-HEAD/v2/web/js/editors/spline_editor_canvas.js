...
comfy.defs.extend('SplineEditor', (b) => {
  BaseEditorCanvas.setupNodeHooks(b);
  b.onCreated((node, created) => {
    BaseEditorCanvas.setupNode(node, created, {
      ...
      menuItems: {
        "Display control lines":  { toggle: (ed) => ed.drawHandles, action: (ed) => { ed.drawHandles = !ed.drawHandles; comfy.settings.set("KJNodes.splineEditor.showControlLines", ed.drawHandles); ed.render(); } },
        "Display sample points":  { toggle: (ed) => ed.drawSamplePoints, action: (ed) => { ed.drawSamplePoints = !ed.drawSamplePoints; comfy.settings.set("KJNodes.splineEditor.showSamplePoints", ed.drawSamplePoints); ed.updatePath(); } },
        "Display arrows":         { toggle: (ed) => ed.showChevrons, action: (ed) => { ed.showChevrons = !ed.showChevrons; comfy.settings.set("KJNodes.splineEditor.showArrows", ed.showChevrons); ed.render(); } },
...
class SplineEditor extends BaseEditorCanvas {
  constructor(context, reset = false) {
    ...
    this.drawSamplePoints = comfy.settings.get("KJNodes.splineEditor.showSamplePoints") ?? false;
    this.drawHandles = comfy.settings.get("KJNodes.splineEditor.showControlLines") ?? false;
    this.showChevrons = comfy.settings.get("KJNodes.splineEditor.showArrows") ?? true;
    this.drawRuler = true;
    ...
    this._widgetListeners = [
      this.interpolationWidget.on('change', () => this.updatePath(true)),
      this.samplingMethodWidget.on('change', () => {
      ...
      this.tensionWidget.on('change', () => this.updatePath(true)),
      this.pointsWidget.on('change', () => this.updatePath(true)),
    ];
    this.setupSizeCallbacks();
    this._widgetListeners.push(this.pointsStoreWidget.on('change', () => {
      if (this._writingPointsStore) return;
    ...
    if (!reset && this.pointsStoreWidget.getValue() !== "") {
      this.parseSplineData();
    } else {
      this.initializeDefaultSplines();
      this.writePointsStore();
    }
  ...
  get interpolation() { return this.interpolationWidget.getValue(); }
  get tension() { return this.tensionWidget.getValue(); }
  get points_to_sample() { return this.pointsWidget.getValue(); }
  get samplingMethod() { return this.samplingMethodWidget.getValue(); }

  // ─── Base class hooks ───

  destroy() {
    super.destroy();
    for (const stopListening of this._widgetListeners) stopListening();
  ...
  writePointsStore() {
    this._writingPointsStore = true;
    this.pointsStoreWidget.setValue(JSON.stringify(this.splines));
    this._writingPointsStore = false;
  }

  parseSplineData() {
    try {
      const parsed = JSON.parse(this.pointsStoreWidget.getValue());
  ...
  updatePath(allDirty = false) {
    ...
    this.writePointsStore();
    if (this.coordWidget) this.coordWidget.setValue(JSON.stringify(this._sampledCache));
    this.render();
  }
