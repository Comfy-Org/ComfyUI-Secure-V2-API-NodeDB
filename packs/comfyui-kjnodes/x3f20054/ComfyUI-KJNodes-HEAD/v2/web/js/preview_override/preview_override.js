...
// Per-node state, keyed by node id. Handles hold no arbitrary properties, so
// the handler lives here and the entry is dropped in onRemoved.
const povNodes = new Map();

// The pack's own backend event, not the built-in preview channel: it carries
// sigmas, per-step timings and a base64 frame, none of which b.onPreview
// exposes. backend.on delivers the raw payload and leaves correlation to us.
comfy.backend.on("kj_preview_override", (data) => {
    if (!data || data.node_id == null) return;
    // The backend names a node by its execution id, which for a node inside a
    // subgraph is the instance path "12:7:5". The old code walked that path from
    // the root graph through each parent's .subgraph; subgraph INSTANCES are not
    // published (comfy.graph.subgraphs() enumerates definitions), so the leaf —
    // the node's own id within whatever graph holds it — is what is matched, and
    // only this pack's own nodes are ever registered here.
    //
    // LIMITATION: node ids are unique per graph, not per document, so two
    // ModelPreviewOverrideKJ nodes carrying the same local id in two different
    // graphs are indistinguishable and the frame goes to whichever registered.
    // The previous conversion matched the whole path against a local id, so a
    // nested node received nothing at all.
    const parts = String(data.node_id).split(":");
    povNodes.get(parts[parts.length - 1])?.handler(data);
});
...
comfy.defs.extend("ModelPreviewOverrideKJ", (b) => {
        b.onCreated((node) => {
            ensureStyles();
            ...
            function applyPanelHeight() {
                const savedH = node.getProperty("kjPovPanelH");
            ...
            grip.addEventListener("mousedown", (e) => {
                ...
                const move = (ev) => {
                    ...
                    // The zoom factor is not published on its own, by design: a
                    // rendered rect already accounts for it, so the ratio against
                    // the unscaled layout box is the scale. Same derivation the API
                    // documents for getScreenRect() against getBounds().
                    const scale = rootRect.height / (root.offsetHeight || 1) || 1;
                ...
                const up = () => {
                    ...
                    node.setProperty("kjPovPanelH", panel.offsetHeight);
                };
            ...
            const fpsWidget = node.widgets.get("preview_fps");
            function currentFps() {
                const v = +fpsWidget?.getValue();
            ...
            // A mounted element owns its listeners, timers and object URLs, so
            // the teardown that was two onRemoved chains lives in destroy().
            node.widgets.mount({
                name: "preview",
                serialize: false,
            ...
            node.setSize({ width: Math.max(size.width, 360), height: Math.max(size.height, 480) });
            ...
            povNodes.set(node.id, { handler, applyPanelHeight });
        });

        b.onConfigured((node) => {
            povNodes.get(node.id)?.applyPanelHeight();
        });

        b.onRemoved((node) => {
            povNodes.delete(node.id);
        });
});
