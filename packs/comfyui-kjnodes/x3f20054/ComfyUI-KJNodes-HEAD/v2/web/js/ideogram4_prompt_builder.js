...
import { addWheelPassthrough, cursorForBboxMode, watchImageInputs } from './utility.js';
// Zoom factor, from how large the node draws on screen versus its size in graph
// units. The viewport transform itself stays the renderer's business.
function viewScale(node) {
  const screen = node.getScreenRect(), graph = node.getBounds();
  return (screen && graph && graph.width) ? screen.width / graph.width : 1;
}
// A client point in graph units, using the node's own on-screen rectangle as
// the reference frame.
function clientToGraph(node, clientX, clientY) {
  const screen = node.getScreenRect(), graph = node.getBounds();
  if (!screen || !graph || !screen.width) return null;
  const scale = screen.width / graph.width;
  return { x: graph.x + (clientX - screen.x) / scale, y: graph.y + (clientY - screen.y) / scale };
}
...
// Node handles hold no arbitrary properties, so the editor's per-node state lives here,
// keyed by node id; the entry is dropped in onRemoved.
const nodeState = new Map();
function stateFor(id) {
  let s = nodeState.get(id);
  if (!s) nodeState.set(id, (s = {}));
  return s;
}
...
try {
  ...
  comfy.backend.on("execution_success", () => {
    for (const n of livePreviewNodes) stateFor(n.id)._ideoGrabFinal?.();
  });
} catch (e) {}
function resultViewUrl(img) {
  const p = new URLSearchParams({ filename: img.filename || "", subfolder: img.subfolder || "", type: img.type || "output" });
  return comfy.backend.url("/view?" + p.toString());
}
...
function pushPreview(img) { for (const n of livePreviewNodes) stateFor(n.id)._ideoSetLiveBg?.(img); }
...
try {
  // Not b.onPreview: these frames belong to whichever node is sampling, not to the editor
  // nodes that display them, so the raw channel is the one that answers this question.
  comfy.backend.on("b_preview", (detail) => feedPreviewBlob(detail));
  comfy.backend.on("b_preview_with_metadata", (detail) => feedPreviewBlob(detail?.blob));
} catch (e) {}
...
// Named templates the user authors, kept per-user on the server so they follow
...
async function listTemplateNames() {
  try {
    const items = await comfy.storage.list(TPL_DIR);
    return items.map((path) => path.split(/[\\/]/).pop() || "")
...
async function loadTemplate(name) {
  try { return (await comfy.storage.get(tplFile(name))) ?? null; } catch (e) {}
  return null;
}
async function saveTemplate(name, caption) {
  try {
    await comfy.storage.set(tplFile(name), caption);
    return true;
  } catch (e) { window.alert("Couldn't save the template to the server."); return false; }
}
async function deleteTemplate(name) {
  try { await comfy.storage.remove(tplFile(name)); } catch (e) {}
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (ae && (ae.tagName === "TEXTAREA" || ae.tagName === "INPUT" || ae.isContentEditable)) return;
  e.preventDefault(); e.stopPropagation();
  stateFor(hoveredCanvasNode.id)._toggleHideBoxes?.();
}, true);
// Alt held over a canvas: preview the alt-click cycle target even without moving the mouse.
document.addEventListener("keydown", (e) => { if (hoveredCanvasNode && e.key === "Alt") stateFor(hoveredCanvasNode.id)._altRefresh?.(true); }, true);
document.addEventListener("keyup", (e) => { if (hoveredCanvasNode && e.key === "Alt") stateFor(hoveredCanvasNode.id)._altRefresh?.(false); }, true);
...
// `isDeleted` resolves a handle against the graph on SCREEN, so a node sitting in a
// subgraph nobody has opened reads as gone. Its own graph is named by node.graphId and
// reachable through subgraphs(), which is where it is really looked for.
function stillInDocument(n) {
  if (!n.isDeleted) return true;
  const owner = comfy.graph.subgraphs().find((sg) => sg.id === n.graphId);
  return !!owner?.node(n.id);
}
...
function sweepOrphanDocks() {
  for (const n of liveDocks) {
    if (stillInDocument(n)) continue;                  // still registered in its graph
    const s = stateFor(n.id);
    try { s._dockRO?.disconnect(); } catch (e) {}
    try { s._visObserver?.disconnect(); } catch (e) {}
    s._dockEl?.remove();
...
function dockAnchor(n) {
  // A handle only resolves inside the graph on screen, so this is the published spelling
  // of the old `n.graph === c.graph`.
  if (!n.isDeleted) return { host: n, ctx: "self" };
  // DROPPED: "Expose to parent" — a dock that follows the SubgraphNode wrapping its
  // subgraph when the user is looking at the outer graph. That needs the subgraph
  // instance, and subgraphs() enumerates definitions: one placed three times has one
  // entry and three wrappers, so the wrapper cannot be derived from the definition.
  // The dock hides rather than following up, which is what it already did whenever the
  // node was in a graph other than the one on screen.
  return null;
}
// gr.x/gr.y = offset (graph units) from host.pos (host = the node, or its SubgraphNode at parent level);
// size (w/h) always lives in dockGraph.
function applyDockTransform(n, anchor) {
  const s = stateFor(n.id);
  const fl = s._dockEl;
  if (!fl) return;
  ...
  s._dockHost = host; s._dockCtx = anchor.ctx;                  // drag/snap handlers read these
  let gr = anchor.ctx === "parent" ? n.getProperty("dockParent") : n.getProperty("dockGraph");
  if (anchor.ctx === "parent" && !gr) { gr = { x: 0, y: host.getSize().height + 2 }; n.setProperty("dockParent", gr); }
  if (!gr) return;
  let hostEl = null;
  if (host.id != null) {                                        // cache; re-query if the host changed or it's gone
    // Feature-detected rather than asking which renderer is running: the query
    // simply finds nothing under a renderer that publishes no node elements.
    if (s._dockHostId !== host.id || !s._dockHostEl || !s._dockHostEl.isConnected) {
      s._dockHostEl = document.querySelector(`[data-node-id="${host.id}"]`);
      s._dockHostId = host.id; s._dockSig = "";
    }
    hostEl = s._dockHostEl;
  }
  if (hostEl) {                                                 // host inside the host element → inherits its transform
    // Element top is the title bar; host.pos sits below it. The strip's height
    // is the gap between the node's rectangle and its body — the renderer's own
    // numbers, so it stays right for a collapsed node.
    const title = host.getPosition().y - host.getBounds().y;
    ...
    if (s._dockSig !== sig) { fl.style.left = gr.x + "px"; fl.style.top = (title + gr.y) + "px"; s._dockSig = sig; }
    return;
  }
  // Legacy / fallback (no Vue element, e.g. just after a mode switch): body-fixed from host.pos + transform.
  // The host's own on-screen rectangle is the reference frame, so the viewport's pan and
  // zoom never have to be read: screen.x is where graph x = bounds.x lands, and the ratio
  // of the two widths is the scale.
  if (fl.parentElement !== document.body) { document.body.appendChild(fl); fl.style.left = ""; fl.style.top = ""; s._dockSig = ""; }  // re-attach if orphaned
  const hostPos = host.getPosition();
  const hostScreen = host.getScreenRect(), hostGraph = host.getBounds();
  if (!hostPos || !hostScreen || !hostGraph || !hostGraph.width) return;
  const scale = hostScreen.width / hostGraph.width;
  const baseLeft = hostScreen.x + (hostPos.x + gr.x - hostGraph.x) * scale;
  const baseTop = hostScreen.y + (hostPos.y + gr.y - hostGraph.y) * scale;
  const tf = `translate(${baseLeft}px,${baseTop}px) scale(${scale})`;
  if (s._dockSig !== tf) { fl.style.position = "fixed"; fl.style.transformOrigin = "top left"; fl.style.transform = tf; s._dockSig = tf; }
  const order = comfy.graph.nodes().findIndex((m) => m.id === host.id);   // sit at the node DOM-widget layer (not above everything)
...
function applyDockTheme(n) {
  const s = stateFor(n.id);
  const fl = s._dockEl; if (!fl) return;
  const pinned = n.isPinned();
  const color = n.getColor(), bgcolor = n.getBgColor();
  const sig = (color || "") + "|" + (bgcolor || "") + "|" + pinned;
  if (s._dockTheme === sig) return;
  s._dockTheme = sig;
  const set = (k, v) => (v ? fl.style.setProperty(k, v) : fl.style.removeProperty(k));
  set("--kj-dock-bg", bgcolor);
  set("--kj-dock-head", color);
  set("--kj-dock-border", color || bgcolor);
  fl.classList.toggle("pinned", pinned);
}
// Event-driven loop: woken by viewport changes (pan/zoom/resize), self-stops when settled.
...
function tickDocks() {
  if (pinnedDocks.size) {
    // Whether node elements exist at all. A renderer swap rebuilds the node DOM,
    // and the same test detects it without asking which renderer is running.
    const vue = document.querySelector("[data-node-id]") != null;
    if (vue !== _dockLastMode) {                  // flip rebuilds the node DOM — force re-parent + re-place
      _dockLastMode = vue;
      for (const n of pinnedDocks) { const s = stateFor(n.id); s._dockSig = ""; s._dockHostEl = null; s._dockHostId = null; }
    }
    for (const n of pinnedDocks) {                // anchor = node (own graph on screen) or its SubgraphNode (exposed)
      const s = stateFor(n.id);
      applyDockTheme(n);
      // "Am I inside a subgraph" is now askable, but the button opts into an anchor
      // dockAnchor() cannot resolve — see the gap there — so it stays hidden.
      if (s._exBtn) s._exBtn.style.display = "none";
      const anchor = dockAnchor(n);
      if (s._dockEl) s._dockEl.style.display = anchor ? "" : "none";
      if (anchor) applyDockTransform(n, anchor);
    }
  }
...
function installDockWakes() {     // pan, zoom and canvas resize all move a pinned dock
  if (_dockWakesInstalled) return;
  _dockWakesInstalled = true;
  // Left subscribed for the process: the sweep it drives also retires docks that are
  // merely floating, so there is no point at which no dock could still need it.
  comfy.onViewportChanged(() => { sweepOrphanDocks(); wakeDocks(); });
}
...
// clone() configures a copy before adding it, so the payload arrives while the only
// handle that resolves is the SOURCE node's. It is held here for the onCreated that
// follows immediately after — one slot, because clone-then-add is a single synchronous
// pair and a stale entry would otherwise be applied to the next node created.
let _clonedConfigure;
function holdClonedConfigure(o) { _clonedConfigure = o; }
function takeClonedConfigure() {
  const held = _clonedConfigure;
  _clonedConfigure = undefined;
  return held;
}

comfy.defs.extend("Ideogram4PromptBuilderKJ", (b) => {
  injectStyle();

  b.onCreated((node, created) => {
    const state = stateFor(node.id);
    // Paste and workflow load both add the node before configuring it, so onCreated
    // runs first and onConfigured addresses the right node. A DUPLICATE does not:
    // clone() configures the copy before it joins the graph, while it still carries the
    // id it was copied from, so that payload arrives at onConfigured under the SOURCE
    // node's handle. `restored` is what names the case, and the payload is held for it.
    state._awaitingConfigure = !created.restored;
    // configure follows add synchronously for both paste and load, so a flag still set
    // after this turn will never be answered — and leaving it set would let a LATER
    // clone's payload, which arrives under this node's id, be applied here.
    if (state._awaitingConfigure) queueMicrotask(() => { state._awaitingConfigure = false; });
    const cloned = created.restored ? takeClonedConfigure() : undefined;
    ...
    if (bgBrightnessWidget && typeof bgBrightnessWidget.getValue() !== "number") bgBrightnessWidget.setValue(25);
    ...
    state._boxes = [];        // {x,y,w,h normalized 0-1, type, text, desc, palette[]}
    state._stylePalette = []; // global style color palette (hex[])
    state._activeIdx = -1;
    state._drawing = false;
    state._placing = false;   // duplicate-placement mode: active box follows the cursor until clicked
    state._dragMode = null;
    state._dragStartN = null; // mouse-down point, normalized
    state._boxAtStart = null; // active box snapshot at drag start
    state._selection = new Set();   // selected box indices (multi-select); always contains _activeIdx
    state._groupStart = null;       // {idx: {x,y,w,h}} snapshot of all selected boxes at drag start
    state._pendingCollapse = -1;    // box to collapse selection to if a click (not drag) on a multi-selection
    state._marquee = null;          // {x0,y0,x,y} rubber-band selection rect (shift-drag), normalized
    state._hoverTitle = null; // index of the title chip under the cursor
    state._hoverBox = null;   // index of the box under the cursor
    state._focused = false;   // editor (DOM) focused — gates the active-box highlight
    state._selected = false;  // node selected in the graph
    state._bgImg = null;      // optional reference image shown as the canvas background
    state._bgManual = false;  // bg set via "use last result" (not the image input)
    state._lastImported = ""; // last import_json applied to the editor (avoid re-apply)
    state._areaObservers = [];             // (reserved) live ResizeObservers to disconnect on rebuild
    ...
    const compactLbl = outToggle(
      () => (formatWidget?.getValue()) !== "pretty",
      (on) => { if (formatWidget) formatWidget.setValue(on ? "compact" : "pretty"); },
      "compact JSON", "Compact output (the format Ideogram 4 expects). Uncheck for pretty/indented.");
    const absLbl = outToggle(
      () => (coordWidget?.getValue()) === "absolute",
      (on) => { if (coordWidget) coordWidget.setValue(on ? "absolute" : "normalized"); },
    ...
    const xyLbl = outToggle(
      () => (orderWidget?.getValue()) === "xy",
      (on) => { if (orderWidget) orderWidget.setValue(on ? "xy" : "yx"); },
    ...
    grabBtn.addEventListener("click", () => { (state._bgManual && state._bgImg) ? state._clearBg() : state._grabResultBg(); });
    function updateGrabBtn() {
      const clear = state._bgManual && state._bgImg;
    ...
    bgSlider.value = bgBrightnessWidget ? bgBrightnessWidget.getValue() : 25;
    bgSlider.title = "Background brightness (image or blank canvas)";
    bgSlider.style.cssText = "width:64px;flex:0 0 auto;";
    stopProp(bgSlider);
    bgSlider.addEventListener("input", () => { if (bgBrightnessWidget) bgBrightnessWidget.setValue(parseInt(bgSlider.value, 10)); drawCanvas(); });
    ...
    guideSel.value = node.getProperty("guide") || "none";
    stopProp(guideSel);
    guideSel.addEventListener("change", () => { node.setProperty("guide", guideSel.value); drawCanvas(); });
    ...
    gridSlider.value = GRID_INV - (node.getProperty("gridSize") || 10);
    ...
    stopProp(gridSlider);
    gridSlider.addEventListener("input", () => { node.setProperty("gridSize", GRID_INV - parseInt(gridSlider.value, 10)); drawCanvas(); });
    ...
    snapChk.type = "checkbox"; snapChk.checked = !!node.getProperty("snap");
    snapChk.addEventListener("change", () => { node.setProperty("snap", snapChk.checked); });
    ...
    guideColor.type = "color"; guideColor.value = node.getProperty("guideColor") || "#ffffff";
    guideColor.style.cssText = "width:32px;height:20px;flex:0 0 auto;padding:0;border:1px solid #555;background:none;";
    stopProp(guideColor);
    guideColor.addEventListener("input", () => { node.setProperty("guideColor", guideColor.value); drawCanvas(); });
    const opacitySlider = document.createElement("input");
    opacitySlider.type = "range"; opacitySlider.min = "0"; opacitySlider.max = "100"; opacitySlider.step = "1";
    opacitySlider.value = node.getProperty("guideOpacity") == null ? 100 : node.getProperty("guideOpacity");
    opacitySlider.style.cssText = "width:90px;flex:0 0 auto;";
    opacitySlider.title = "Guide/grid line opacity";
    stopProp(opacitySlider);
    opacitySlider.addEventListener("input", () => { node.setProperty("guideOpacity", parseInt(opacitySlider.value, 10)); drawCanvas(); });
    ...
    const txtToggle = (prop, label, title) => {
      const l = document.createElement("label");
      l.style.cssText = "display:flex;align-items:center;gap:4px;cursor:pointer;"; l.title = title || "";
      const cb = document.createElement("input"); cb.type = "checkbox"; cb.checked = node.getProperty(prop) !== false;
      cb.addEventListener("change", () => { node.setProperty(prop, cb.checked); drawCanvas(); });
      l.appendChild(cb); l.appendChild(document.createTextNode(label));
      l._cb = cb; return l;
    };
    ...
    sizeSlider.value = node.getProperty("textSize") || 12;
    sizeSlider.style.cssText = "width:90px;flex:0 0 auto;";
    sizeSlider.title = "Region text font size";
    stopProp(sizeSlider);
    sizeSlider.addEventListener("input", () => { node.setProperty("textSize", parseInt(sizeSlider.value, 10)); drawCanvas(); });
    ...
    boxOpacSlider.value = node.getProperty("boxOpacity") == null ? 14 : node.getProperty("boxOpacity");
    boxOpacSlider.style.cssText = "width:90px;flex:0 0 auto;";
    boxOpacSlider.title = "Box fill opacity";
    stopProp(boxOpacSlider);
    boxOpacSlider.addEventListener("input", () => { node.setProperty("boxOpacity", parseInt(boxOpacSlider.value, 10)); drawCanvas(); });
    ...
    state._panelH = state._panelH || 150;                    // height of the prompt panel (set by the splitter)
    panel.style.height = state._panelH + "px";
    ...
    splitter.addEventListener("pointerdown", (e) => {
      ...
      e.preventDefault(); e.stopPropagation();
      const scale = node.getProperty("dockPinned") ? viewScale(node) : 1;
      ...
      dragPointer(e, splitter, (me) => {
        ...
        state._panelH = Math.round(h); panel.style.height = state._panelH + "px";   // persisted via the onSerialize blob
        fitCanvas();
      }, flushChange);
    });
    ...
    // The old code asserted node.resizable = true here, which is already the default,
    ...
    try {
      state._visObserver = new IntersectionObserver((entries) => {
        if (entries.some((en) => en.isIntersecting)) fitCanvas();
      });
      state._visObserver.observe(wrap);
    } catch (e) {}
    ...
    function fitCanvas() {
      ...
      const aspect = (wWidget?.getValue() || 1) / (hWidget?.getValue() || 1);
    ...
    function detachInto(container) {                        // move the editor element into a container (dock body / fullscreen)
      state._wrapHome = wrap.parentNode;
      container.appendChild(wrap);
      state._fsInner = container;
    }
    function reattach() {                                   // fullscreen exit → back into the floating dock
      canvasEl.style.width = ""; canvasEl.style.height = "";
      const home = state._dockBody || state._wrapHome;
      if (home) home.appendChild(wrap);
      state._fsInner = state._dockBody || null;               // stays detached, living in the dock
    ...
    function exitFs() {
      ...
      state._fsOverlay?.remove(); state._fsOverlay = null;
      reattach();
    }
    ...
    function dockPos() {
      return state._dockCtx === "parent" ? (node.getProperty("dockParent") || node.getProperty("dockGraph")) : node.getProperty("dockGraph");
    }
    // The node the dock is currently anchored to: the node itself, or the SubgraphNode at parent level.
    function dockHostNode() { return state._dockHost || node; }
    // Persist the panel geometry — screen-space rect when unpinned, graph-space size when pinned.
    function saveDockGeom() {
      const fl = state._dockEl; if (!fl) return;
      if (node.getProperty("dockMin")) return;              // collapsed: don't persist the header-only size
      ...
      if (node.getProperty("dockPinned")) {
        const g = node.getProperty("dockGraph") || { x: 0, y: 0 };
        g.w = fl.offsetWidth; g.h = fl.offsetHeight;
        node.setProperty("dockGraph", g);
      } else {
        const r = fl.getBoundingClientRect();
        node.setProperty("dockRect", { x: Math.round(r.left), y: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height) });
      }
    }
    ...
    function setPinned(on, pinBtn) {
      const fl = state._dockEl;
      if (!fl) return;
      if (on) {                                          // return to where it was pinned (the saved dockGraph)
        let g = node.getProperty("dockGraph");
        if (!g) {                                        // never pinned before — derive from the current float position
          const r = fl.getBoundingClientRect();
          const gp = clientToGraph(node, r.left, r.top);
          if (!gp) return;                               // nothing on screen to measure against
          const scale = viewScale(node);
          const pos = node.getPosition();
          g = {
            x: gp.x - pos.x, y: gp.y - pos.y,
            w: Math.round(fl.offsetWidth / scale), h: Math.round(fl.offsetHeight / scale),
          };
          node.setProperty("dockGraph", g);
        }
        node.setProperty("dockPinned", true);
        fl.style.left = ""; fl.style.top = "";
        fl.style.width = g.w + "px"; fl.style.height = g.h + "px";
        state._dockSig = ""; state._dockHostEl = null; state._dockHostId = null;     // force a fresh place + re-parent
        ...
        const r = fl.getBoundingClientRect();
        node.setProperty("dockPinned", false);
        pinnedDocks.delete(node);
        state._dockSig = ""; state._dockHostEl = null; state._dockHostId = null;
    ...
    function startDockResize(e, dir, fl) {
      if (e.button !== 0 || node.isPinned()) return;       // node pinned → not resizable, like the node
      e.preventDefault(); e.stopPropagation();
      const pinned = !!node.getProperty("dockPinned");
      const scale = pinned ? viewScale(node) : 1;
      ...
      dragPointer(e, e.currentTarget, (me) => {
        ...
        if (pinned) { const p = dockPos(); p.x = gx; p.y = gy; state._dockSig = ""; applyDockTransform(node); }
    ...
    function undock() {
      if (state._docked) return;
      if (state._fullscreen) exitFs();
      state._docked = true; node.setProperty("docked", true);
      ...
      minBtn.addEventListener("click", () => { applyMin(!node.getProperty("dockMin")); flushChange(); });
      ...
      stopProp(fsBtn); fsBtn.addEventListener("click", () => state._fullscreen ? exitFs() : enterFs());
      const pinBtn = document.createElement("button"); pinBtn.className = "kjideo-btn"; pinBtn.textContent = "📌";
      stopProp(pinBtn); pinBtn.addEventListener("click", () => setPinned(!node.getProperty("dockPinned"), pinBtn));
      ...
      // Hidden for the same reason tickDocks keeps it hidden: the anchor it opts into
      // cannot be resolved. See the gap in dockAnchor().
      exBtn.style.display = "none";
      const syncExBtn = () => {
        exBtn.classList.toggle("active", !!node.getProperty("exposeToParent"));
        exBtn.title = (node.getProperty("exposeToParent") ? "Hide" : "Show") + " this canvas at the parent (subgraph) level";
      };
      stopProp(exBtn);
      exBtn.addEventListener("click", () => { node.setProperty("exposeToParent", !node.getProperty("exposeToParent")); syncExBtn(); state._dockSig = ""; wakeDocks(); flushChange(); });
      syncExBtn();
      state._syncExBtn = syncExBtn; state._exBtn = exBtn;     // onConfigure refreshes state; tickDocks toggles visibility
      ...
      state._dockEl = fl; state._dockBody = body; state._dockSig = ""; state._dockHostEl = null; state._dockHostId = null; liveDocks.add(node);
      applyDockTheme(node);                                 // match the node's color theme right away
      detachInto(body);
      // initial geometry / mode
      const pinned = !!node.getProperty("dockPinned");
      if (pinned) {
        const g = node.getProperty("dockGraph") || { x: 0, y: 0, w: 540, h: 470 };
        ...
        const rc = node.getProperty("dockRect") || { x: 90, y: 90, w: 540, h: 470 };
      ...
      applyMin(!!node.getProperty("dockMin"));              // restore minimized state
      ...
      head.addEventListener("pointerdown", (e) => {
        ...
        if (node.isPinned()) return;                       // node pinned → dock is locked too
        e.preventDefault();
        ...
        dragPointer(e, head, (me) => {
          if (pinned) {                                  // move the panel by writing its transform directly (no full redraw)
            ...
            const snap = 26 / scale, underY = h.getSize().height + 2;
          ...
          if (snapReady) {                               // commit the snap: flush under the anchor, matching its width
            const h = dockHostNode(), w = Math.round(h.getSize().width), p = dockPos();
            p.x = 0; p.y = h.getSize().height + 2;
            const dg = node.getProperty("dockGraph");
            if (dg.w !== w) { dg.w = w; fl.style.width = w + "px"; }
            state._dockSig = ""; applyDockTransform(node);
          }
      ...
      try { state._dockRO = new ResizeObserver(() => { fitFsCanvas(); saveDockGeom(); }); state._dockRO.observe(fl); } catch (e) {}
    ...
    function ensureDocked(fresh) {
      if (state._docked) return;
      if (node.getProperty("dockPinned") == null) node.setProperty("dockPinned", true);
      const honor = !!node.getProperty("dockGraph") && state._dockGeomRestored;   // only trust blob-restored geometry
      if (!honor) {
        // old workflow: reuse its saved (aspect-locked) node height so the canvas isn't tiny
        const sh = (!fresh && state._savedSize && state._savedSize[1] > 240) ? Math.round(state._savedSize[1]) : 470;
        node.setProperty("dockGraph", { x: 0, y: 0, w: Math.round(node.getSize().width), h: sh });   // match the node width
      }
      undock();
      if (!honor) requestAnimationFrame(() => {             // no reliable geometry — place a default under the node, matching its width
        const g = node.getProperty("dockGraph");
        g.x = 0; g.y = node.getSize().height + 2; g.w = Math.round(node.getSize().width);
        if (state._dockEl) state._dockEl.style.width = g.w + "px";
        state._dockSig = ""; applyDockTransform(node); saveDockGeom();
      });
    }
    ...
    function boxesAt(mN) {
      ...
      const res = [];
      for (let i = 0; i < state._boxes.length; i++) {
        const b = state._boxes[i];
      ...
      const ai = res.findIndex((c) => c.index === state._activeIdx);
    ...
    function hitTest(mN) {
      ...
      return cands.find((c) => c.index === state._activeIdx && c.mode !== "move") || cands[0];
    }
    ...
    function titleAt(mN) {
      ...
      for (let i = state._boxes.length - 1; i >= 0; i--) {
        if (state._boxes[i].locked) continue;             // locked badges aren't interactive
        const r = rects[i];
    ...
    function textBlocks() {
      if (node.getProperty("showBoxText") === false) return [];
      const { fs, lh } = txtFont();
      ctx.font = fs + "px monospace";
      const W = logW(), H = logH(), pad = 4, tagH = 14, auto = node.getProperty("textAutoPlace") !== false;
      const placed = [], blocks = [];
    ...
    function pickForSelection(mN, cycle) {
      const cands = boxesAt(mN);
      if (!cands.length) return null;
      const ah = cands.find((c) => c.index === state._activeIdx && c.mode !== "move");
      ...
      if (cycle && cands.length > 1) {
        ...
        const pos = ordered.findIndex((c) => c.index === state._activeIdx);
    ...
    // ── grid / guides ──
    function gridN() { return Math.max(2, Math.min(128, node.getProperty("gridSize") || 10)); }
    function txtFont() { const fs = Math.max(6, Math.min(40, node.getProperty("textSize") || 12)); return { fs, lh: Math.round(fs * 1.2) }; }
    ...
    function guideStroke(a) {
      const c = hexRgb(node.getProperty("guideColor") || "#ffffff") || { r: 255, g: 255, b: 255 };
      const op = (node.getProperty("guideOpacity") == null ? 100 : node.getProperty("guideOpacity")) / 100;
    ...
    function snapBox(b, mode) {
      if (!node.getProperty("snap")) return b;
    ...
    function drawGuide(W, H) {
      const kind = node.getProperty("guide");
    ...
    function _draw() {
      ...
      let bri = bgBrightnessWidget ? bgBrightnessWidget.getValue() : 25;
      if (typeof bri !== "number" || isNaN(bri)) bri = 25;       // guard against unset widget value
      if (state._bgImg) {                                         // reference image, dimmed by brightness
      ...
      if (state._hideBoxes) return;                              // H: temporary background-only view
      // selection highlight only when the editor is focused or the node is selected
      const showSel = state._focused || state._selected;
      const aIdx = showSel ? state._activeIdx : -1;
      const selSet = new Set(showSel ? state._selection : []);
      if (aIdx >= 0) selSet.add(aIdx);
      // index 0 = front (drawn last); selected drawn above non-selected, active last of all
      const nonSel = state._boxes.map((_, i) => i).filter((i) => !selSet.has(i)).reverse();
      const selOthers = state._boxes.map((_, i) => i).filter((i) => selSet.has(i) && i !== aIdx).reverse();
      const order = [...nonSel, ...selOthers];
      if (aIdx >= 0 && aIdx < state._boxes.length) order.push(aIdx);
      const tagR = tagRects();                              // collision-avoided tag positions
      const textB = textBlocks();                           // collision-avoided in-box text positions
      for (const i of order) {
        const b = state._boxes[i], active = i === aIdx, selected = selSet.has(i) && !b.locked;  // locked never shows as selected
        ...
        const hovered = (i === state._hoverBox && !b.locked) || selected;  // locked boxes don't hover; selected stay highlighted
        if (selected) {                                    // opaque backing so contents read clearly over boxes behind
          ctx.fillStyle = "rgba(26,26,26,0.88)";
          ctx.fillRect(x1, y1, w, h);
        }
        const baseA = (node.getProperty("boxOpacity") == null ? 14 : node.getProperty("boxOpacity")) / 100;
        ...
        const tb = textB[i];
        if (tb) {
          const stroke = node.getProperty("textStroke") !== false;
        ...
        if (i === state._hoverTitle && !b.locked) {          // hover highlight (never on locked)
      ...
      const apIdx = state._altPreview ?? -1;                 // Alt-held: ring the box an alt-click would select next
      if (apIdx >= 0 && apIdx < state._boxes.length) {
        const { x1, y1, x2, y2 } = toPx(state._boxes[apIdx]);
        ctx.strokeStyle = "#46b4e6"; ctx.lineWidth = 2; ctx.setLineDash([5, 3]);
        ctx.strokeRect(x1 + 1, y1 + 1, (x2 - x1) - 2, (y2 - y1) - 2);
        ctx.setLineDash([]);
      }
      if (state._marquee && state._marqueeActive) {           // rubber-band selection rectangle
        const r = marqueeRect();
    ...
    const importConnected = () => {
      const src = node.inputs.byName("import_json")?.source();
      return !!src && !["never", "bypass"].includes(comfy.graph.node(src.nodeId)?.getMode());
    };
    function serialize() {                              // saved/restored value: clean boxes
      if (elementsWidget) elementsWidget.setValue(state._boxes.length ? JSON.stringify(state._boxes) : "");
      if (stylePaletteWidget) stylePaletteWidget.setValue(state._stylePalette.length ? JSON.stringify(state._stylePalette) : "");
    }
    ...
    // 'prompt' only: the saved workflow keeps the real boxes, so reloading a file
    // never restores a nonce. The widget itself is untouched either way.
    if (elementsWidget) {
      elementsWidget.on("beforeSerialize", (e) => {
        if (e.context !== "prompt") return;
        const always = findW("import_mode")?.getValue() === "always";
        if (!importConnected() || !(always || !state._boxes.length)) return;
        e.setSerializedValue(JSON.stringify({ _refresh: (state._serialSeq = (state._serialSeq || 0) + 1) }));
      });
    }
    ...
    function removeBox(i) {
      state._boxes.splice(i, 1);
      state._selection = new Set();
      if (state._boxes.length === 0) state._activeIdx = -1;
      else if (i <= state._activeIdx) state._activeIdx = Math.max(0, state._activeIdx - 1);
      if (state._activeIdx >= 0) state._selection.add(state._activeIdx);
    }
    // Delete every selected box (or the active one) and reset the selection.
    function removeSelected() {
      const idxs = [...state._selection].sort((a, b) => b - a).filter((i) => !state._boxes[i]?.locked);  // keep locked
      if (!idxs.length && state._activeIdx >= 0 && !state._boxes[state._activeIdx]?.locked) idxs.push(state._activeIdx);
      if (!idxs.length) return;
      for (const i of idxs) state._boxes.splice(i, 1);
      state._selection = new Set();
      state._activeIdx = state._boxes.length ? Math.min(idxs[idxs.length - 1], state._boxes.length - 1) : -1;
      if (state._activeIdx >= 0) state._selection.add(state._activeIdx);
    }
    // Replace the selection with a single box (used by clicks / programmatic selects).
    function selectOnly(idx) {
      state._activeIdx = idx;
      state._selection = idx >= 0 ? new Set([idx]) : new Set();
    }

    // ── pointer interaction ──
    canvasEl.addEventListener("pointerdown", (e) => {
      if (state._placing) {             // drop the duplicate being placed
      ...
      if (e.button !== 0) return;
      if (state._hideBoxes) return;     // view-only while boxes are hidden (H)
      ...
      state._hoverTitle = null; state._hoverBox = null;  // clear hover highlight while interacting
      const mN = mouseN(e);
      ...
      if (e.pointerType && e.pointerType !== "mouse") {
        const last = state._lastTap, now = e.timeStamp;
        if (hit && last && now - last.t < 350 && Math.abs(mN.x - last.x) < 0.03 && Math.abs(mN.y - last.y) < 0.03) {
          state._lastTap = null; openInlineEditor(hit.index);
          e.preventDefault(); e.stopPropagation(); return;
        }
        state._lastTap = { t: now, x: mN.x, y: mN.y };
      }
      state._pendingCollapse = -1;
      state._groupStart = null;
      ...
      if (hit) {
        if (!state._selection.has(hit.index)) state._selection = new Set([hit.index]);  // outside selection → pick it
        else if (state._selection.size > 1) state._pendingCollapse = hit.index;          // click (no drag) collapses
        state._activeIdx = hit.index;
        state._dragMode = hit.mode;
        state._boxAtStart = { ...state._boxes[hit.index] };
        if (state._selection.size > 1) {                  // snapshot the whole group for group move/resize
          state._groupStart = {};
          for (const i of state._selection) state._groupStart[i] = { ...state._boxes[i] };
        }
      } else {
        state._dragMode = "draw";
        const nb = { x: mN.x, y: mN.y, w: 0, h: 0, type: "obj", text: "", desc: "", palette: [] };
        state._boxes.push(nb);
        state._activeIdx = state._boxes.length - 1;
        state._selection = new Set([state._activeIdx]);
        state._boxAtStart = { ...nb };
      }
      state._drawing = true;
      state._dragStartN = mN;
    ...
    function applyHover(t) {
      if (t.ti !== state._hoverTitle || t.hb !== state._hoverBox || t.ap !== (state._altPreview ?? -1)) {
        state._hoverTitle = t.ti; state._hoverBox = t.hb; state._altPreview = t.ap; drawCanvas();
      }
    }
    canvasEl.addEventListener("pointermove", (e) => {
      state._lastMouseN = mouseN(e);                        // track cursor for paste-under-cursor
      if (state._placing) { placeFollower(state._lastMouseN); return; }
      if (state._drawing || state._marquee || state._hideBoxes) return;
      const mN = state._lastMouseN;
    ...
    state._altRefresh = (altDown) => {
      if (state._drawing || state._marquee || state._placing || state._hideBoxes || !state._lastMouseN) return;
      applyHover(hoverTargets(state._lastMouseN, altDown, false));
    };
    canvasEl.addEventListener("pointerleave", () => {
      if (hoveredCanvasNode === node) hoveredCanvasNode = null;
      if (state._hoverTitle !== null || state._hoverBox !== null || (state._altPreview ?? -1) >= 0) {
        state._hoverTitle = null; state._hoverBox = null; state._altPreview = -1; drawCanvas();
      }
    });
    ...
    state._toggleHideBoxes = () => { state._hideBoxes = !state._hideBoxes; drawCanvas(); };
    ...
    canvasEl.addEventListener("dblclick", (e) => {
      if (state._hideBoxes) return;
      e.preventDefault(); e.stopPropagation();
      const cands = boxesAt(mouseN(e));     // edit the active box if it's under the cursor, else topmost
      const target = cands.find((c) => c.index === state._activeIdx) || cands[0];
      if (target) openInlineEditor(target.index);
    });

    // Snapshot the current selection (or the active box) for the clipboard.
    function copySelection() {
      const idxs = state._selection.size ? [...state._selection].sort((a, b) => a - b)
        : (state._activeIdx >= 0 ? [state._activeIdx] : []);
      return idxs.map((i) => JSON.parse(JSON.stringify(state._boxes[i])));
    }
    ...
    function pasteBoxes() {
      ...
      const start = state._boxes.length;
      for (const b of clones) {
      ...
      state._selection = new Set();
      for (let i = start; i < state._boxes.length; i++) state._selection.add(i);
      state._activeIdx = state._boxes.length - 1;
      commit(); fitCanvas();
    }
    ...
    canvasEl.addEventListener("keydown", (e) => {
      ...
      if (state._drawing || state._hideBoxes) return;   // view-only while boxes are hidden (H)
      const ctrl = e.ctrlKey || e.metaKey;
      if ((e.key === "Delete" || e.key === "Backspace") && state._activeIdx >= 0) {
        e.preventDefault(); e.stopPropagation();
        removeSelected(); commit(); fitCanvas();      // removes all selected (or the active one)
      } else if (ctrl && e.key === "c" && state._activeIdx >= 0) {
        e.preventDefault(); e.stopPropagation();
        copiedBoxes = copySelection();
      } else if (ctrl && e.key === "v" && copiedBoxes) {
        e.preventDefault(); e.stopPropagation();
        pasteBoxes();
      } else if (ctrl && e.key === "d" && state._activeIdx >= 0) {
        e.preventDefault(); e.stopPropagation();
        copiedBoxes = copySelection();
        pasteBoxes();
      }
    ...
    function onMove(e) {
      ...
      const mN = mouseN(e);
      const dN = { x: mN.x - state._dragStartN.x, y: mN.y - state._dragStartN.y };
      if (Math.abs(dN.x) + Math.abs(dN.y) > 0.001) state._pendingCollapse = -1;  // it's a drag, not a click
      if (state._dragMode === "move" && state._groupStart) {
        let dx = dN.x, dy = dN.y;                   // clamp delta so the whole group stays in bounds
        for (const i in state._groupStart) {
          const s = state._groupStart[i];
          dx = Math.min(Math.max(dx, -s.x), 1 - s.w - s.x);
          dy = Math.min(Math.max(dy, -s.y), 1 - s.h - s.y);
        }
        if (node.getProperty("snap")) {             // snap the group's movement to whole grid cells
          const { sx, sy } = gridStep();
        ...
        for (const i in state._groupStart) {
          const s = state._groupStart[i];
          state._boxes[i] = { ...s, x: s.x + dx, y: s.y + dy };
          delete state._boxes[i].nobbox;
        }
        drawCanvas(); updateBboxLabel();
        return;
      }
      if (state._groupStart && state._dragMode.startsWith("resize")) {
        // Scale every selected box by the primary's resize, about the handle's fixed edge.
        const a = state._boxAtStart, na = applyDrag(state._dragMode, a, dN);
        const suf = state._dragMode.slice(7);       // "tl"|"tr"|"bl"|"br"|"t"|"b"|"l"|"r"
      ...
      const nb = snapBox(applyDrag(state._dragMode, state._boxAtStart, dN), state._dragMode);
      delete nb.nobbox;            // moving/resizing places the element (gives it a bbox)
      state._boxes[state._activeIdx] = nb;
      drawCanvas(); updateBboxLabel();
    }
    function onUp() {
      if (!state._drawing) return;
      ...
      const b = state._boxes[state._activeIdx];
      if (b && (b.w < 0.005 || b.h < 0.005) && state._dragMode === "draw") {
        state._boxes.splice(state._activeIdx, 1);
        selectOnly(-1);
      } else if (state._pendingCollapse >= 0) {     // click (no drag) on a group member → keep only it
        selectOnly(state._pendingCollapse);
      }
      state._pendingCollapse = -1; state._groupStart = null;
      commit();
    }
    ...
    function startMarquee(mN, startHit) {
      state._marquee = { x0: mN.x, y0: mN.y, x: mN.x, y: mN.y };
      state._marqueeBase = new Set(state._selection);   // additive: marquee unions with what's selected
      state._marqueeStartHit = startHit;               // for the shift-click (no drag) toggle fallback
      state._marqueeActive = false;
      canvasEl.focus();
    ...
    function onMarqueeMove(e) {
      if (!state._marquee) return;
      const mN = mouseN(e);
      state._marquee.x = mN.x; state._marquee.y = mN.y;
      if (Math.abs(mN.x - state._marquee.x0) + Math.abs(mN.y - state._marquee.y0) > 0.01) state._marqueeActive = true;
      if (state._marqueeActive) {
        const r = marqueeRect();
        const sel = new Set(state._marqueeBase);
        state._boxes.forEach((b, i) => { if (!b.locked && rectsOverlap(r, b)) sel.add(i); });   // marquee skips locked
        state._selection = sel;
        if (state._activeIdx < 0 || !sel.has(state._activeIdx)) state._activeIdx = sel.size ? [...sel][0] : state._activeIdx;
      }
      drawCanvas();
    }
    function onMarqueeUp() {
      ...
      if (!state._marqueeActive && state._marqueeStartHit >= 0) {   // shift-click on a box → toggle it
        const idx = state._marqueeStartHit;
        if (state._selection.has(idx) && state._selection.size > 1) {
          state._selection.delete(idx);
          if (state._activeIdx === idx) state._activeIdx = state._selection.values().next().value;
        } else {
          state._selection.add(idx); state._activeIdx = idx;
        }
      }
      state._marquee = null; state._marqueeActive = false;
      if (state._activeIdx >= 0 && !state._selection.has(state._activeIdx)) {
        state._activeIdx = state._selection.size ? [...state._selection][0] : -1;
      }
      commit();
    }
    ...
    function cancelPlacing() {
      ...
      state._placing = false;
      canvasEl.style.cursor = "crosshair";
      removeBox(state._activeIdx);
      commit(); fitCanvas();
    }

    // ── right-click "layers" menu: list / select / delete / duplicate / reorder regions ──
    function closeLayersMenu() {
      if (state._layerMenu) { state._layerMenu.remove(); state._layerMenu = null; }
      state._layerDismiss?.disarm(); state._layerDismiss = null;
    }
    function rowLabel(b) {
      if (b.type === "text") {
    ...
    function openLayersMenu(clientX, clientY) {
      ...
      function buildRows() {
        list.innerHTML = "";
        if (!state._boxes.length) {
          const empty = document.createElement("div");
          empty.className = "kjideo-mhdr"; empty.textContent = "No regions yet.";
          list.appendChild(empty);
          return;
        }
        state._boxes.forEach((b, i) => {
          const row = document.createElement("div");
          row.className = "kjideo-lrow" + (i === state._activeIdx ? " active" : "");
          row._box = b;
          ...
          lock.addEventListener("click", (e) => {
            ...
            if (b.locked) {                              // a locked box drops out of the selection
              const idx = state._boxes.indexOf(b);
              state._selection.delete(idx);
              if (state._activeIdx === idx) state._activeIdx = state._selection.size ? [...state._selection][0] : -1;
            }
            commit(); buildRows();
          });
          ...
          row.addEventListener("click", () => {
            ...
            selectOnly(state._boxes.indexOf(b));
            commit();
          ...
          del.addEventListener("click", (e) => {
            ...
            if (!state._boxes.length) { closeLayersMenu(); return; }
            buildRows();
          });
          ...
          row.addEventListener("pointerdown", (e) => {
            ...
            const up = () => {
              ...
              if (dragging) {
                ...
                const active = state._boxes[state._activeIdx];
                const order = Array.from(list.querySelectorAll(".kjideo-lrow")).map((el) => el._box);
                if (order.length === state._boxes.length) state._boxes = order;
                selectOnly(active ? state._boxes.indexOf(active) : -1);  // reorder invalidates multi-select indices
                renumber();
                commit();
              }
            };
      ...
      state._layerDismiss = outsideDismiss(menu, () => closeLayersMenu());
      state._layerDismiss.arm();
    }

    canvasEl.addEventListener("contextmenu", (e) => {
      e.preventDefault(); e.stopPropagation();
      if (state._placing || state._hideBoxes) return;
      closeInlineEditor();
    ...
    clearBtn.addEventListener("click", () => {
      closeInlineEditor();
      state._boxes = []; state._activeIdx = -1; state._selection = new Set(); state._stylePalette = [];
      state._lastImported = "";
    ...
    function getW(name) { const w = findW(name); return w ? w.getValue() : ""; }
    ...
    function captionBboxJS(b) {
      const abs = (coordWidget?.getValue()) === "absolute", xy = (orderWidget?.getValue()) === "xy";
      const sx = abs ? (wWidget?.getValue() || 1000) : 1000, sy = abs ? (hWidget?.getValue() || 1000) : 1000;
    ...
    function buildCaption() {
      ...
      const kind = styleW ? styleW.getValue() : "none";
      if (kind !== "none") {
        ...
        const pal = cleanPalette(state._stylePalette);
        if (pal.length) sd.color_palette = pal;
        cap.style_description = sd;
      }
      const elements = state._boxes.map((b) => {
      ...
      return (formatWidget?.getValue()) === "pretty" ? pyJson(cap) : JSON.stringify(cap);   // compact by default; matches the node output
    }
    ...
    function applyCaption(cap) {
      ...
      state._boxes = els.map((el, i) => bboxElemToBox(el, i)).filter(Boolean);
      selectOnly(state._boxes.length ? 0 : -1);
      ...
      const styleW = findW("style");
      if (styleW) styleW.setValue(kind);          // setter synchronously rebuilds sub-widgets
      ...
      state._stylePalette = Array.isArray(sd.color_palette) ? sd.color_palette.slice() : [];
    }
    ...
    function insertCaptionBoxes(cap) {
      ...
      const start = state._boxes.length;
      state._boxes.push(...added);
      state._selection = new Set();                      // select the inserted regions
      for (let i = start; i < state._boxes.length; i++) state._selection.add(i);
      state._activeIdx = state._boxes.length - 1;
      commit(); fitCanvas();
    }
    ...
    function applyImported(capStr) {
      ...
      const always = findW("import_mode")?.getValue() === "always";
      if (capStr === state._lastImported && !always) return;
      const cap = tryParseCaption(capStr);
      if (!cap) return;
      state._lastImported = capStr;
      loadCaption(cap);
    }
    state._onExecuted = function (message) {
      ...
      if (message?.boxes && !state._boxes.length) {
        const seeded = JSON.parse(message.boxes[0]);
        if (Array.isArray(seeded) && seeded.length) {
          state._boxes = seeded.filter((b) => b && typeof b.x === "number" && typeof b.w === "number");
          selectOnly(state._boxes.length ? 0 : -1);
          commit(); fitCanvas();
        }
      }
      ...
      if (message?.dims && !state._bgImg) {
        const [w, h] = message.dims;
        if (wWidget && w) wWidget.setValue(w);
        if (hWidget && h) hWidget.setValue(h);
    ...
    function rebuildStylePalette() {
      ...
      buildSwatchRow(styleBar, state._stylePalette, MAX_STYLE_COLORS,
        swatchEdit,
    ...
    function dims() { return [wWidget ? wWidget.getValue() : 1024, hWidget ? hWidget.getValue() : 1024]; }
    ...
    function commitPxEdit() {
      const b = state._boxes[state._activeIdx]; if (!b || !bboxPx) return;
    ...
    function commitGridEdit() {
      const b = state._boxes[state._activeIdx]; if (!b || !bboxGrid) return;
    ...
    function renderPanel() {
      for (const ro of state._areaObservers) ro.disconnect();
      state._areaObservers = [];
      panel.innerHTML = "";
      const b = state._boxes[state._activeIdx];
      if (!b) {
        hint.textContent = "";
        ...
        p.style.color = "#888";
        p.textContent = state._boxes.length ? "Click a region to edit it." : "No regions yet.";
        panel.appendChild(p);
        return;
      }
      const col = (b.palette || []).find(Boolean) || "#bbb";
      const selN = state._selection.size;
      ...
      tag.style.color = col; tag.textContent = "region " + (state._activeIdx + 1);
    ...
    for (const w of [wWidget, hWidget]) {
      if (!w) continue;
      w.on("change", () => { syncCanvasToDims(); drawCanvas(); fitCanvas(); updateTokens(); });  // absolute coords depend on w/h
    }
    ...
    function loadBg(src, keepAspect) {
      ...
      img.onload = () => {
        ...
        if (!keepAspect) {                                          // live mode keeps the user's canvas aspect
          ...
          if (wWidget) wWidget.setValue(r16(img.naturalWidth));      // match canvas aspect to the image
          if (hWidget) hWidget.setValue(r16(img.naturalHeight));
        }
        syncCanvasToDims(); drawCanvas(); fitCanvas(); updateGrabBtn();
      };
      img.src = src;
    }
    // watchImageInputs used to chain node.onConnectionsChange and node.onRemoved onto
    // the instance from inside itself; both are declared per node TYPE now, so it
    // returns its two subscription points and the hooks at the bottom of this file
    // drive them.
    //
    // DROPPED: a connected video used to be previewed by grabbing a frame off
    // VideoHelperSuite's own <video> element — REFUSED, see resolveSourcePreview in
    // utility.js. A video source now reaches the background only through its widget
    // value or the source node's executed images.
    state._sourceWatch = watchImageInputs(node, "image", (sources) => {
      if (!sources.length) { if (!state._bgManual) { state._bgImg = null; drawCanvas(); updateGrabBtn(); } return; }
      state._bgManual = false;                              // a connected image takes over
      const s = sources[0];
      if (s.url) loadBg(s.url);
    });
    ...
    state._grabResultBg = () => {
      ...
      state._bgManual = true;
      loadBg(resultViewUrl(lastResultImage));
    };
    state._clearBg = () => {
      state._bgManual = false; state._bgImg = null;
      if (state._liveBmp?.close) { try { state._liveBmp.close(); } catch (e) {} state._liveBmp = null; }
      drawCanvas(); updateGrabBtn();
    };
    // Feed a live sampling-preview frame as the background (no width/height change).
    state._ideoSetLiveBg = (bmp) => {
      if (state._liveBmp?.close && state._liveBmp !== bmp) { try { state._liveBmp.close(); } catch (e) {} }
      state._liveBmp = bmp; state._bgImg = bmp; state._bgManual = true;
      drawCanvas(); updateGrabBtn();
    };
    // After generation, replace the live preview with the full-res final result.
    state._ideoGrabFinal = () => {
    ...
    wrap.addEventListener("focusin", () => { if (!state._focused) { state._focused = true; drawCanvas(); } });
    wrap.addEventListener("focusout", (e) => {
      if (!wrap.contains(e.relatedTarget)) { state._focused = false; finishPlacing(); closeLayersMenu(); drawCanvas(); }
    });
    // DROPPED: the active-box highlight used to follow node selection as well as editor
    // focus. comfy.graph.selection() reads the current selection but nothing announces a
    // change to it, and polling for one is what this migration removes. The deselect
    // side is not lost — finishing a placement and closing the layers menu now happen on
    // focusout above, which is when the user has actually left the editor.

    state._onRemoved = function () {
      livePreviewNodes.delete(node);
      if (hoveredCanvasNode === node) hoveredCanvasNode = null;
      if (state._fullscreen) {                       // tear down the popup if open
        document.removeEventListener("keydown", onFsEsc, true);
        window.removeEventListener("resize", fitFsCanvas);
        state._fsOverlay?.remove();
      }
      pinnedDocks.delete(node); liveDocks.delete(node);
      state._dockRO?.disconnect(); state._dockEl?.remove();   // tear down the floating dock if open
      state._visObserver?.disconnect();
      if (state._liveBmp?.close) { try { state._liveBmp.close(); } catch (e) {} state._liveBmp = null; }  // release GPU bitmap
      closeBgMenu(); state._bgMenu?.remove();
      closeTxtMenu(); state._txtMenu?.remove();
      closeTplMenu(); state._tplMenu?.remove();
      closeOutMenu(); state._outMenu?.remove();
      closeInlineEditor();
      closeLayersMenu();
      for (const ro of state._areaObservers) ro.disconnect();
      state._areaObservers = [];
    ...
    state._onSerialize = function () {
      const p = node.getProperties();
      return { ideo: { boxes: state._boxes, palette: state._stylePalette, importMode: findW("import_mode")?.getValue(),
        outputFormat: findW("output_format")?.getValue(), coordMode: findW("coord_mode")?.getValue(), bboxOrder: findW("bbox_order")?.getValue(),
        dock: { pinned: p.dockPinned, graph: p.dockGraph, rect: p.dockRect, panelH: state._panelH, min: p.dockMin,
          exposeParent: p.exposeToParent, parent: p.dockParent } } };
    };
    state._onConfigure = function (o) {
      ...
      state._savedSize = (o && Array.isArray(o.size) && o.size.length === 2) ? [o.size[0], o.size[1]] : [node.getSize().width, node.getSize().height];
      const d = o && o.ideo && o.ideo.dock;
      if (d) {
        state._dockGeomRestored = true;                     // honor it; old workflows w/o the blob get a default
        if (d.graph) node.setProperty("dockGraph", d.graph);
        if (d.rect) node.setProperty("dockRect", d.rect);
        if (d.pinned != null) node.setProperty("dockPinned", d.pinned);
        if (d.min != null) node.setProperty("dockMin", d.min);
        if (d.exposeParent != null) node.setProperty("exposeToParent", d.exposeParent);
        if (d.parent) node.setProperty("dockParent", d.parent);
        if (typeof d.panelH === "number") { state._panelH = d.panelH; panel.style.height = d.panelH + "px"; }
      }
      state._syncExBtn?.();                                  // refresh the expose-to-parent toggle after restore
      ...
      let boxes = (o && o.ideo && Array.isArray(o.ideo.boxes)) ? o.ideo.boxes : _parseBoxes(elementsWidget?.getValue() || "");
      if (!boxes) { for (const v of raw) { const b = _parseBoxes(v); if (b) { boxes = b; break; } } }
      if (!boxes) { for (const w of node.widgets.all()) { const b = _parseBoxes(w.getValue()); if (b) { boxes = b; break; } } }
      if (boxes) {
        state._boxes = boxes.filter((b) => b && typeof b.x === "number");
        selectOnly(state._boxes.length ? 0 : -1);
      }
      ...
      if (!pal) { try { const p = JSON.parse(stylePaletteWidget?.getValue() || ""); if (isPal(p)) pal = p; } catch (e) {} }
      if (!pal) { for (const v of raw) { try { const p = JSON.parse(v); if (isPal(p)) { pal = p; break; } } catch (e) {} } }
      if (pal) state._stylePalette = pal.slice();
      ...
      if (imW) {                                            // old workflows (saved before this widget) don't fail Combo validation
        ...
        imW.setValue(opts.includes(im) ? im : (opts.includes(imW.getValue()) ? imW.getValue() : "when empty"));
      }
      if (formatWidget) {                                   // restore output format from the blob; default compact unless explicitly pretty
        formatWidget.setValue((o && o.ideo && o.ideo.outputFormat) === "pretty" ? "pretty" : "compact");
        compactLbl._cb.checked = formatWidget.getValue() === "compact";
      }
      if (coordWidget) {                                    // restore coord mode; default normalized unless explicitly absolute
        coordWidget.setValue((o && o.ideo && o.ideo.coordMode) === "absolute" ? "absolute" : "normalized");
        absLbl._cb.checked = coordWidget.getValue() === "absolute";
      }
      if (orderWidget) {                                    // restore bbox axis order; default yx unless explicitly xy
        orderWidget.setValue((o && o.ideo && o.ideo.bboxOrder) === "xy" ? "xy" : "yx");
        xyLbl._cb.checked = orderWidget.getValue() === "xy";
      }
      state._configured = true;                              // mark as loaded so initial layout keeps the restored size
      hideDataWidgets();
      serialize();                                         // realign widget values for Python + future saves
      if (bgBrightnessWidget) {
        if (typeof bgBrightnessWidget.getValue() !== "number") bgBrightnessWidget.setValue(25);  // old workflows may restore ""
        bgSlider.value = bgBrightnessWidget.getValue();
      }
      // Properties are restored after onCreated, so resync the toolbar controls to them.
      liveChk.checked = !!node.getProperty("liveBg");
      if (liveChk.checked) livePreviewNodes.add(node); else livePreviewNodes.delete(node);
      guideSel.value = node.getProperty("guide") || "none";
      gridSlider.value = GRID_INV - (node.getProperty("gridSize") || 10);
      snapChk.checked = !!node.getProperty("snap");
      guideColor.value = node.getProperty("guideColor") || "#ffffff";
      opacitySlider.value = node.getProperty("guideOpacity") == null ? 100 : node.getProperty("guideOpacity");
      showLbl._cb.checked = node.getProperty("showBoxText") !== false;
      strokeLbl._cb.checked = node.getProperty("textStroke") !== false;
      autoLbl._cb.checked = node.getProperty("textAutoPlace") !== false;
      sizeSlider.value = node.getProperty("textSize") || 12;
      boxOpacSlider.value = node.getProperty("boxOpacity") == null ? 14 : node.getProperty("boxOpacity");
      syncCanvasToDims();
      rebuildStylePalette();
      ...
      if (state._dockGeomRestored && o && Array.isArray(o.size) && o.size.length === 2) node.setSize({ width: o.size[0], height: o.size[1] });
      requestAnimationFrame(() => ensureDocked(false));    // reload: build the dock honoring saved geometry
    };

    // A duplicate was configured before it joined the graph, so its payload was held
    // rather than applied; apply it now that the copy has state to apply it to.
    if (cloned) state._onConfigure(cloned);
    ...
    setTimeout(() => {
      hideDataWidgets();
      if (!state._configured) node.setSize({ width: Math.max(480, node.getSize().width), height: node.getSize().height });   // fresh: comfortable default width, keep natural height
      else if (!state._dockGeomRestored) {                                                                   // old workflow: drop the embedded-editor height, shrink to content
        node.setSize({ width: Math.max(380, node.getSize().width), height: node.getSize().height });
        node.setSizeConstraints({ autoHeight: true });
      }
      else if (node.getSize().width < 380) node.setSize({ width: 380, height: node.getSize().height });     // current reload/paste: keep saved size, enforce min width
      syncCanvasToDims();
      rebuildStylePalette();
      renderPanel();
      drawCanvas();
      updateTokens();
      if (!state._configured) ensureDocked(true);           // fresh node: pop the dock under it, matching its width
    }, 0);
  });

  b.onExecuted((node, result) => stateFor(node.id)._onExecuted?.(result.raw));
  b.onResized((node) => stateFor(node.id)._onResized?.());
  b.onSerialize((node) => stateFor(node.id)._onSerialize?.() ?? {});
  b.onConfigured((node, o) => {
    const state = stateFor(node.id);
    // A clone being configured under an id this pack already owns. clone() adds the
    // copy immediately afterwards, so one slot is enough to carry the payload across.
    if (!state._awaitingConfigure) { holdClonedConfigure(o); return; }
    state._awaitingConfigure = false;
    state._onConfigure?.(o);
  });
  b.onConnectionsChanged((node, event) => {
    if (event.side === "input") stateFor(node.id)._sourceWatch?.refresh();
  });
  b.onRemoved((node) => {
    stateFor(node.id)._sourceWatch?.stop();
    stateFor(node.id)._onRemoved?.();
    nodeState.delete(node.id);
  });
});
