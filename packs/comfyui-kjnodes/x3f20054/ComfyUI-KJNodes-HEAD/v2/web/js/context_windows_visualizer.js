...
// Per-node restore hook, keyed by node id: handles hold no arbitrary properties
// and onConfigured is registered per type, not per instance.
const restoreHooks = new Map();
...
comfy.defs.extend('ContextWindowsVisualizerKJ', (b) => {
    b.onCreated((node) => {
        ...
        node.widgets.mount({
            ...
            // Teardown belongs to the mounted element: the interval, the observer
            // and the document-level pan listener all outlive node removal otherwise.
            destroy: () => {
                stopPlayback();
                ro.disconnect();
            },
    ...
    b.onConfigured((node) => {
        restoreHooks.get(node.id)?.();
    });

    b.onRemoved((node) => {
        restoreHooks.delete(node.id);
    });
});
