...
comfy.defs.extend("ScreencapStream", (b) => {
	b.onCreated((node) => {
		const frameWidget = node.widgets.get("frame_data");
		...
		// --- Widget references ---
		const cropWWidget = node.widgets.get("crop_width");
		const cropHWidget = node.widgets.get("crop_height");
		...
		function updateSizeWidgets() {
			...
			if (dims) {
				cropWWidget.setValue(dims.w);
				cropHWidget.setValue(dims.h);
				// clamp max to full capture dimensions
				cropWWidget.setOption("max", video.videoWidth || dims.w);
				cropHWidget.setOption("max", video.videoHeight || dims.h);
			}
		...
		function updateCropFromWidgets() {
			...
			const newW = Math.min(cropWWidget.getValue(), video.videoWidth);
			const newH = Math.min(cropHWidget.getValue(), video.videoHeight);
		...
		// --- Widget callbacks ---
		cropWWidget.on("change", updateCropFromWidgets);
		cropHWidget.on("change", updateCropFromWidgets);
		...
		function updateBtnTimer() {
			...
			btnWidget.setLabel(`\uD83D\uDD34 ${elapsed} - Stop capture`);
		}
		...
		function stopStream() {
			...
			if (btnWidget) btnWidget.setLabel("Start capture");
		}

		btnWidget = node.widgets.add({ type: "button", name: "Start capture", value: null });
		btnWidget.on("activate", async () => {
		...
		// --- DOM widget ---
		// getMinHeight: () => 200 meant "at least this tall, and grow with the node".
		// MountDef.height is a fixed reservation, so the growable path is the one that
		// matches: omit height and the mount fills the node's spare space, and a node
		// minimum keeps at least the old 200px available for it.
		//
		// COSMETIC: hideOnZoom: false kept the preview drawn when the user zoomed far
		// out, where core hides DOM widgets. There is no published equivalent, so at
		// low zoom the preview follows whatever the active renderer does with mounted
		// elements.
		node.setSizeConstraints({ minHeight: 200 });
		node.widgets.mount({
			name: "preview",
			serialize: false,
		...
		const previewBtn = node.widgets.add({ type: "button", name: "Hide preview", value: null });
		previewBtn.on("activate", () => {
			previewVisible = !previewVisible;
			previewBtn.setLabel(previewVisible ? "Hide preview" : "Show preview");
		...
		// The frame is captured for the queued payload only. `serializeValue` was
		// the prompt builder's hook, so the saved workflow keeps the stored (empty)
		// value rather than a base64 PNG.
		frameWidget.on("beforeSerialize", (e) => {
			if (e.context !== "prompt") return;
			e.setSerializedValue(captureFrameData());
		});
	});
});
