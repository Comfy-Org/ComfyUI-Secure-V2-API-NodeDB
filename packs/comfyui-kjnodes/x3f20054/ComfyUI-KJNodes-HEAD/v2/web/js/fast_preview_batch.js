...
// Per-node handlers. Handles hold no arbitrary properties, so what the node
// closure used to own lives here, keyed by id and dropped on removal.
...
comfy.defs.extend('FastPreviewBatch', (b) => {
    b.onCreated((node) => {
        ...
        function handleExecuted(output) {
        ...
        batchNodes.set(node.id, { handleExecuted, handleConfigured, handleRemoved });
    });

    b.onExecuted((node, result) => {
        batchNodes.get(node.id)?.handleExecuted(result.raw);
    });

    b.onConfigured((node) => {
        batchNodes.get(node.id)?.handleConfigured();
    });

    b.onRemoved((node) => {
        batchNodes.get(node.id)?.handleRemoved();
        batchNodes.delete(node.id);
    });
});
