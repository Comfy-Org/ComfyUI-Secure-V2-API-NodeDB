...
import { addWheelPassthrough } from './utility.js';

// Shared across all HDR Preview nodes so synced nodes can drive each other.
const hdrSyncGroup = new Set();

// Per-node state and handlers. Handles hold no arbitrary properties, so what
// used to hang off the node lives here, keyed by id and dropped on removal.
...
comfy.defs.extend("HDRPreviewKJ", (b) => {
    b.onCreated((node) => {
        ...
        let widgetHeight = controlsHeight;
        ...
        // COSMETIC: margin: 0 and hideOnZoom: false were legacy DOM-widget options —
        // the container's inset, and staying drawn when the user zooms far out. Neither
        // has a published equivalent, so the row sits at the host's default inset and
        // follows whatever the active renderer does with mounted elements at low zoom.
        const previewWidget = node.widgets.mount({
            name: "hdr_preview",
            render: (el) => el.appendChild(container),
        });

        // Fitting the node to its content is declared, not summed. The manual
        // total existed because the renderer's size computation was believed to
        // ignore DOM widgets; it consults their layout size, so asking it to
        // fit is both shorter and correct for hidden and widget-backed rows.
        // Re-declaring replaces the constraint rather than stacking another.
        //
        // The row itself is still pinned: without a stated height the node divides
        // its spare space between every unpinned widget, which is what the old
        // getMinHeight/getMaxHeight/getHeight trio existed to prevent. setHeight is
        // re-callable, so it tracks widgetHeight as the viewport is re-fitted.
        function fitNodeToContent() {
            previewWidget.setHeight(widgetHeight);
            node.setSizeConstraints({ autoHeight: true });
        }

        // Don't gate on state.frames — a fire during async bitmap load would wipe widgetHeight and collapse the node.
        const controlsObserver = new ResizeObserver(() => {
            ...
            widgetHeight = viewportPx + controlsHeight;
            fitNodeToContent();
        });
        ...
        // The mounted element spans the node's width, so observing it is how a resize is heard.
        ...
        async function handleExecuted(message) {
            ...
            // LIMITATION: the old code followed this write with node.graph.change() and
            // changeTracker.checkState() to flag the workflow modified straight away.
            // Neither is published — the host captures state on the user's next
            // interaction — so after a run the modified flag, and any autosave keyed on
            // it, lags by one interaction. What is written is unaffected: an explicit
            // save serializes the live graph, property and all.

            await applyPreviewData(data);
        }

        function handleConfigured() {
            if (exposureWidget && isFinite(exposureWidget.getValue())) state.exposure = exposureWidget.getValue();
            if (saturationWidget && isFinite(saturationWidget.getValue())) state.saturation = saturationWidget.getValue();
            const saved = node.getProperty("hdrLastPreview");
        ...
        hdrNodes.set(node.id, { state, handleExecuted, handleConfigured, handleRemoved });
    });

    b.onExecuted((node, result) => {
        hdrNodes.get(node.id)?.handleExecuted(result.raw);
    });

    b.onConfigured((node) => {
        hdrNodes.get(node.id)?.handleConfigured();
    });

    b.onRemoved((node) => {
        hdrNodes.get(node.id)?.handleRemoved();
