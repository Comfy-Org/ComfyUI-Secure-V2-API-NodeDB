...
import { getSlotPos, typesCompatible } from "./utility.js";
...
function typeMatchTier(outType, inType) {
	if (!outType || !inType) return 1;   // an untyped slot accepts anything
	if (!typesCompatible(outType, inType)) return -1;
...
function orderNodes(nodes) {
	...
	for (const node of nodes) {
		for (const out of node.outputs.all()) {
			for (const link of out.links()) {
				if (selectedIds.has(link.targetNodeId) && link.targetNodeId !== node.id) {
					if (!outEdges.get(node.id).has(link.targetNodeId)) {
						outEdges.get(node.id).add(link.targetNodeId);
	...
	const bySpatial = (a, b) => a.getPosition().x - b.getPosition().x || a.getPosition().y - b.getPosition().y;
...
function planConnections(ordered) {
	...
	for (let b = 1; b < ordered.length; b++) {
		...
		for (let inIdx = 0; inIdx < nodeB.inputs.length; inIdx++) {
			const inp = nodeB.inputs.at(inIdx);
			if (inp.isConnected) continue;

			const inPos = getSlotPos(nodeB, true, inIdx);
			// The renderer places slots; one it cannot place has no distance.
			if (!inPos) continue;
			...
			for (let a = b - 1; a >= 0; a--) {
				...
				for (let outIdx = 0; outIdx < nodeA.outputs.length; outIdx++) {
					const out = nodeA.outputs.at(outIdx);
...
function connectPlanned(planned) {
	for (const p of planned) {
		p.sourceNode.outputs.at(p.outIdx).connectTo(p.targetNode.id, { index: p.inIdx });
	}
}

function fillConnectSelected() {
	if (!comfy.settings.get("KJNodes.fillConnectEnabled")) return;

	const nodes = comfy.graph.selection();
