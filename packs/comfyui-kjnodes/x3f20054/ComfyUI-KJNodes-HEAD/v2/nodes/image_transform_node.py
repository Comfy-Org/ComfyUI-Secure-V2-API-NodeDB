...
Use extra_padding to add padding with color or edge fill (clamp/repeat/mirror).""",
    ...
    @classmethod
    async def execute(cls, image, target_width, target_height, upscale_method, keep_proportion, divisible_by,
                extra_padding, invert_crop, bboxes, mask=None):
        image = await image.raw()
        mask = None if mask is None else await mask.raw()
        ...
        # Preview for the JS canvas: brokered. The host writes its own temp
        # file and hands back a name; no host path enters this process.
        preview_payload = await sdk.ctx().ui.preview_images(
            await sdk.ImageRef._from_raw(image[:1]))
        preview_entries = preview_payload.get("images") if isinstance(preview_payload, dict) else None
        if not preview_entries:
            raise RuntimeError("ctx.ui.preview_images returned no image descriptors")
        preview_ui = {"preview_filename": [preview_entries[0]["filename"]]}
        ...
        out_ref = sdk.MaskRef if input_is_mask else sdk.ImageRef
        all_cropped = [await out_ref._from_raw(t) for t in all_cropped]
        all_cropped_masks = [await sdk.MaskRef._from_raw(t) for t in all_cropped_masks]
        all_bbox_masks = [await sdk.MaskRef._from_raw(t) for t in all_bbox_masks]
...
class BBOXToBoundingBoxKJ(io.ComfyNode):
    ...
    @classmethod
    async def execute(cls, bbox, bbox_format, index) -> io.NodeOutput:
        boxes = normalize_bboxes(bbox, bbox_format)
        return io.NodeOutput(bbox_to_bounding_box(boxes[min(index, len(boxes) - 1)], bbox_format))


NODE_CLASS_MAPPINGS = {
    "ImageTransformKJ": ImageTransformKJ,
    "BBOXToBoundingBoxKJ": BBOXToBoundingBoxKJ,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "ImageTransformKJ": "Image Transform KJ",
    "BBOXToBoundingBoxKJ": "BBOX to Bounding Box KJ",
}
