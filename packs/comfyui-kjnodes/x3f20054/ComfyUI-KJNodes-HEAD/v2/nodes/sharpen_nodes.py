...
Uses radius as the estimated blur kernel and iterations to control convergence.""",
    ...
    @classmethod
    async def execute(cls, image, method) -> io.NodeOutput:
        ...
        return io.NodeOutput(await type(image_ref)._from_raw(result))
...
def _rcas(image: torch.Tensor, strength: float) -> torch.Tensor:
    ...
    device = image.device
    intermediate_device = image.device
    dtype = image.dtype
...
NODE_CLASS_MAPPINGS = {
    "ImageSharpenKJ": ImageSharpenKJ,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "ImageSharpenKJ": "Image Sharpen KJ",
}
