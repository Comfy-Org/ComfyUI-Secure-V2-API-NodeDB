"""Frozen schema and wire helpers for the pinned Image Saver conversion."""
...
_MODEL_COMBOS = {
    ...
    "diffusion_models": "/models/diffusion_models/choices",
}


def _catalogue_for_input(node_id: str, input_id: str) -> str | None:
    if node_id == "Checkpoint Loader with Name (Image Saver)" and input_id == "ckpt_name":
        return "checkpoints"
    if node_id == "UNet loader with Name (Image Saver)" and input_id == "unet_name":
...
def schema_for(node_id: str) -> io.Schema:
    ...
    attrs["hidden"] = []
    if node_id == "Any to String (Image Saver)":
