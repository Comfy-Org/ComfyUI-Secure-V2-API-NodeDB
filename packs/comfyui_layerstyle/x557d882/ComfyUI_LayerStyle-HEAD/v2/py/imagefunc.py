...
try:
    ...
    def guidedFilter(guide, source, radius, epsilon):
        """Portable grayscale guided filter used when ximgproc is absent."""
        ...
        mean_guide = cv2.boxFilter(
            guide, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT)
        mean_source = cv2.boxFilter(
            source, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT)
        guide_values = guide[..., None] if source.ndim == 3 else guide
        correlation = cv2.boxFilter(
            guide_values * source, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT)
        variance = cv2.boxFilter(
            guide * guide, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT) - mean_guide * mean_guide
        if source.ndim == 3:
            mean_guide = mean_guide[..., None]
            variance = variance[..., None]
        coefficient = (
            correlation - mean_guide * mean_source
        ) / (variance + max(float(epsilon), 1e-8))
        offset = mean_source - coefficient * mean_guide
        mean_coefficient = cv2.boxFilter(
            coefficient, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT)
        mean_offset = cv2.boxFilter(
            offset, -1, (size, size), normalize=True,
            borderType=cv2.BORDER_REFLECT)
        return mean_coefficient * guide_values + mean_offset
...
class _CubeLUT:
    """Small Iridas .cube evaluator used by the pack's LUT node."""

    def __init__(self, table, title, domain, comments, dimensions):
    ...
    def apply(self, values):
        ...
        normalized = np.clip(
            (values[..., :3] - self.domain[0]) / scale, 0.0, 1.0
        )
        size = self.table.shape[0]
        position = normalized * (size - 1)
        ...
        if self.dimensions == 1:
            ...
            for channel in range(3):
                result[..., channel] = (
                    self.table[low[..., channel], channel]
                    * (1.0 - fraction[..., channel])
                    + self.table[high[..., channel], channel]
                    * fraction[..., channel]
                )
            return result

        r0, g0, b0 = (low[..., index] for index in range(3))
        r1, g1, b1 = (high[..., index] for index in range(3))
        rf, gf, bf = (fraction[..., index, None] for index in range(3))
        ...
        c00 = c000 * (1.0 - rf) + c100 * rf
        c10 = c010 * (1.0 - rf) + c110 * rf
        c01 = c001 * (1.0 - rf) + c101 * rf
        c11 = c011 * (1.0 - rf) + c111 * rf
        c0 = c00 * (1.0 - gf) + c10 * gf
        c1 = c01 * (1.0 - gf) + c11 * gf
...
def read_LUT_IridasCube_encode_utf8(path: str):
    ...
    if dimensions == 2:
        if table.shape != (size, 3):
            raise ValueError("invalid 1D .cube table size")
        LUT = _CubeLUT(
            table, title, np.vstack([domain_min, domain_max]), comments, 1
        )
    elif dimensions == 3:
        ...
        if table.shape != (size ** 3, 3):
            raise ValueError("invalid 3D .cube table size")
        table = table.reshape([size, size, size, 3], order="F")
        LUT = _CubeLUT(
            table, title, np.vstack([domain_min, domain_max]), comments, 3
        )
...
def get_resource_dir() -> list:
    pack_root = os.path.dirname(os.path.dirname(os.path.normpath(__file__)))
    default_lut_dir = [os.path.join(pack_root, 'lut')]
    default_font_dir = [os.path.join(pack_root, 'font')]
    LUT_DICT = {}
    for directory in default_lut_dir:
        LUT_DICT.update(collect_files(root_dir=directory, suffixes=('.cube',), default_dir=default_lut_dir[0]))

    FONT_DICT = {}
    for directory in default_font_dir:
        FONT_DICT.update(collect_files(root_dir=directory, suffixes=('.ttf', '.otf'), default_dir=default_font_dir[0]))
