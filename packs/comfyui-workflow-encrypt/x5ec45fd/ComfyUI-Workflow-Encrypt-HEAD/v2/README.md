...
3. The encryption key appears once in a browser dialog. It is never sent to or logged by the ComfyUI server.![overall](doc/browser-key.png)
...
3. Click the Load (Decrypted) action-bar button or command. Encrypted files can also be opened or dropped into ComfyUI directly.
...
## Secure Nodes V2 implementation

Fernet framing, key generation, authentication order, base64, timestamps, and JSON serialization remain inside this pack's sandboxed frontend worker. The host supplies fixed, bounded AES-CBC and HMAC-SHA256 primitives, a bounded snapshot of the current workflow, a bounded file picker/download, prompts, and workflow opening. The converted pack has no backend nodes, HTTP routes, runtime package installation, filesystem access, or access to the parent document.
